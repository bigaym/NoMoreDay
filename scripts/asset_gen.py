...
def start_comfyui(use_lowvram=True):
    print(f"ComfyUI not found at {SERVER_ADDRESS}. Starting it now...", flush=True)
    if not os.path.exists(COMFYUI_PATH):
        print(f"Error: Could not find ComfyUI start script at: {COMFYUI_PATH}")
        print(
            "Tip: If you are using a 12GB card like 4070S, ensure your .bat file includes --lowvram"
        )
        sys.exit(1)
    ...
    if use_lowvram:
        print(
            "Note: Flux2 requires massive VRAM. On 12GB cards, it MUST run in --lowvram mode."
        )
    for i in range(30):
...
def get_default_workflow(unet_name="flux-2-klein-9b.safetensors"):
    return {
        ...
        "9": {
            "inputs": {"images": ["8", 0], "filename_prefix": "NoMoreDay_Asset"},
...
def get_available_models():
    try:
        response = requests.get(
            f"http://{SERVER_ADDRESS}/object_info/CheckpointLoaderSimple"
        )
...
def main():
    ...
    parser.add_argument(
        "--prompt", type=str, required=True, help="Positive prompt for the asset"
    )
    parser.add_argument(
        "--negative",
        type=str,
        default="blur, low quality, 3d render, text, background",
    ...
    parser.add_argument(
        "--name", type=str, default="asset", help="Output filename prefix"
    )
    parser.add_argument(
        "--no-remove-bg",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Final asset width after downscaling",
    )
    parser.add_argument(
        "--target-height",
        type=int,
        default=128,
        help="Final asset height after downscaling",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=os.path.join("assets", "textures"),
    ...
    print(
        f"Workflow Configuration: Generation Res={gen_width}x{gen_height}, Prompt='{args.prompt}'"
    )

    # Update Prompts and Dimensions
    workflow["6"]["inputs"]["text"] = (
        f"{args.prompt}, white background, isolated, 2d game sprite"
    )
    ...
    image_paths = get_images(
        ws, workflow, output_path, args.name, not args.no_remove_bg
    )
    ...
    for img_path in image_paths:
        try:
            with Image.open(img_path) as img:
                print(
                    f"Standardizing {os.path.basename(img_path)} to {args.target_width}x{args.target_height}..."
                )
