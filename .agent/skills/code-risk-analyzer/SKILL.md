...
- **内存安全**: 使用 `search` 与 `analyze {mode:'semantic'}` 识别违反 `conductor/code_standard.md` 1.1 节的行为（特别是 EnTT 指针失效）。
...
1.  **全量扫描**:
    ...
    - **静态脚本**: `powershell -File .gemini/skills/code-risk-analyzer/scripts/deep_scan.ps1`。
2.  **生成报告**:
...
3.  **记忆锚定**: 
    - 使用 `memory {operation:'anchor', anchor_type:'pattern'}` 记录发现的高危代码模式，供未来自动识别。
