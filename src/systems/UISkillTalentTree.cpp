...
void UISkillTalentTree::Draw(entt::registry& registry, entt::entity player, uint32_t skillId) {
    ...
    // Draw Background
    DrawRectangleRec({startX, startY, panelW, panelH}, Fade(BLACK, 0.95f * alpha));
    DrawRectangleLinesEx({startX, startY, panelW, panelH}, 2.0f, Fade(DARKGRAY, alpha));

    // Header
    UISystem::DrawTextUI(TextFormat("%s - 专精天赋", skillData->name_key.c_str()), startX + 20, startY + 20, 30, GOLD, alpha);
    
    // Back Button
    Rectangle backRect = {startX + panelW - 120, startY + 20, 100, 40};
    bool backHover = CheckCollisionPointRec(GetMousePosition(), backRect);
    DrawRectangleRec(backRect, Fade(backHover ? GRAY : DARKGRAY, alpha));
    UISystem::DrawTextUI("返回 (Hub)", backRect.x + 10, backRect.y + 10, 18, WHITE, alpha);
    ...
    // Points
    UISystem::DrawTextUI(TextFormat("可用点数: %d", active->available_talent_points), startX + 20, startY + 60, 20, WHITE, alpha);
    ...
    for (const auto& [id, node] : tree->nodes) {
        ...
        for (uint32_t preId : node.prerequisites) {
            if (tree->nodes.count(preId)) {
                ...
                DrawLineEx(start, end, 3.0f, Fade(prePts > 0 ? GOLD : DARKGRAY, alpha));
            }
        }
    }
    ...
    for (const auto& [id, node] : tree->nodes) {
        ...
        DrawCircleV(pos, nodeRadius, Fade(BLACK, 0.8f * alpha));
        DrawCircleLinesV(pos, nodeRadius, Fade(nodeColor, alpha));
        if (currentPts > 0) {
            DrawCircleV(pos, nodeRadius * 0.8f, Fade(nodeColor, 0.3f * alpha));
        }

        // Icon (Placeholder)
        UISystem::DrawTextUI(TextFormat("%d/%d", currentPts, node.max_points), pos.x - 15, pos.y - 8, 14, WHITE, alpha);
        ...
        if (hovered) {
            ...
            DrawRectangle(tx, ty, 200, 100, Fade(BLACK, 0.9f * alpha));
            DrawRectangleLines(tx, ty, 200, 100, Fade(nodeColor, alpha));
            UISystem::DrawTextUI(node.name_key.c_str(), tx + 10, ty + 10, 18, GOLD, alpha);
            UISystem::DrawTextUI(node.desc_key.c_str(), tx + 10, ty + 35, 14, WHITE, alpha);
