...
GPUParticleSystemV2& GPUParticleSystemV2::Get() {
    static GPUParticleSystemV2 instance;
    return instance;
}

void GPUParticleSystemV2::Init(int maxParticles) {
    if (m_initialized) {
        LOG_WARN("GPUParticleSystemV2: Already initialized!");
        return;
    }
    
    m_maxParticles = maxParticles;
    LOG_INFO("Initializing GPUParticleSystemV2 with {} max particles...", maxParticles);
    ...
    if (!glDrawArraysIndirectPtr) {
        LOG_ERROR("GPUParticleSystemV2: glDrawArraysIndirect not available!");
        return;
    }
    
    if (!glBindBufferPtr) {
        LOG_ERROR("GPUParticleSystemV2: glBindBuffer not available!");
        return;
    }
    ...
    LOG_INFO("GPUParticleSystemV2: Initialized successfully with Indirect Drawing support.");
}

void GPUParticleSystemV2::Shutdown() {
    if (!m_initialized) return;
    
    LOG_INFO("GPUParticleSystemV2: Shutting down...");
...
void GPUParticleSystemV2::LoadShaders() {
    // 1. Load Compute Shader
    std::ifstream compFile("assets/shaders/particle_v2.compute");
    if (compFile.is_open()) {
        std::stringstream ss;
        ss << compFile.rdbuf();
        ...
        if (compId != 0) {
            ...
            if (m_computeShader.id != 0) {
                LOG_INFO("GPUParticleSystemV2: Compute shader loaded (ID: {})", m_computeShader.id);
                ...
                LOG_ERROR("GPUParticleSystemV2: Compute shader linking failed!");
            }
        } else {
            LOG_ERROR("GPUParticleSystemV2: Compute shader compilation failed!");
        }
    } else {
        LOG_ERROR("GPUParticleSystemV2: Could not open particle_v2.compute");
    }
    
    // 2. Load Render Shaders
    m_renderShader = LoadShader("assets/shaders/particle_v2.vert", "assets/shaders/particle_v2.frag");
    if (m_renderShader.id != 0) {
        LOG_INFO("GPUParticleSystemV2: Render shader loaded (ID: {})", m_renderShader.id);
        m_renderMvpLoc = GetShaderLocation(m_renderShader, "mvp");
    } else {
        LOG_ERROR("GPUParticleSystemV2: Render shader loading failed!");
    }
}

void GPUParticleSystemV2::CreateBuffers() {
    ...
    LOG_DEBUG("GPUParticleSystemV2: Created particle buffer ({} bytes)", totalSize);
    
    // Compact buffer (alive particles only)
    m_compactBuffer.Create(totalSize);
    LOG_DEBUG("GPUParticleSystemV2: Created compact buffer ({} bytes)", totalSize);
    ...
    LOG_DEBUG("GPUParticleSystemV2: Created indirect buffer");
    
    // Atomic counter buffer (4 bytes)
    uint32_t zero = 0;
    m_atomicBuffer.Create(sizeof(uint32_t));
    m_atomicBuffer.Update(&zero, sizeof(zero));
    LOG_DEBUG("GPUParticleSystemV2: Created atomic buffer");
}

void GPUParticleSystemV2::CreateQuadVAO() {
    ...
    LOG_DEBUG("GPUParticleSystemV2: Created quad VAO (ID: {})", m_quadVAO);
}

void GPUParticleSystemV2::Emit(const components::GPUParticle& particle) {
    if (m_stagedParticles.size() < (size_t)m_maxParticles) {
        m_stagedParticles.push_back(particle);
    }
}

void GPUParticleSystemV2::EmitBatch(const std::vector<components::GPUParticle>& particles) {
...
void GPUParticleSystemV2::Update(float dt) {
...
Matrix GPUParticleSystemV2::BuildMVP(const Camera2D& camera) const {
...
void GPUParticleSystemV2::Render(const Camera2D& camera) {
    ...
    // If Update changed pp to true, then prev Out was Compact. pp=true selects Compact. Correct.
