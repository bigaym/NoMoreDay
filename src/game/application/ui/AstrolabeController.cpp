...
void AstrolabeController::Update(entt::registry& registry) {
    (void)registry;
}

void AstrolabeController::Toggle(entt::registry& registry, entt::entity player) {
    (void)registry;
    (void)player;
    m_visible = !m_visible;
    if (m_visible) {
...
bool AstrolabeController::IsVisible(entt::registry& registry, entt::entity player) const {
    (void)registry;
    (void)player;
...
void AstrolabeController::SetNodeVisible(bool visible) {
    if (m_rootNodeId != kInvalidUiId) {
        m_runtime.SetNodeVisible(m_rootNodeId, visible);
    }
}

void AstrolabeController::Draw(entt::registry& registry) {
...
void AstrolabeController::DrawInternal(entt::registry& registry, entt::entity player) {
    EnsureLoaded();
    ...
    // Hit Test
    Vector2 mouseWorld = GetScreenToWorld2D(GetMousePosition(), m_view.camera);
    uint32_t hoverId = 0;
    ...
    m_renderer.Draw(graph, m_view, astroComp, hoverId);
    ...
    // Vow Dialog (Modal)
    if (m_showVowDialog && astroComp && !astroComp->hasVow()) {
        const auto& star = graph.professionStars[(int)m_pendingVowProfession];
        DrawVowDialog(registry, player, star);
    }
}

void AstrolabeController::HandleCameraInput(float dt) {
    (void)dt;
    if (IsMouseButtonDown(MOUSE_RIGHT_BUTTON)) {
        Vector2 delta = GetMouseDelta();
        m_view.camera.target = Vector2Add(m_view.camera.target, Vector2Scale(delta, -1.0f / m_view.camera.zoom));
    }

    float wheel = GetMouseWheelMove();
    if (wheel != 0) {
        using namespace Constants::Astrolabe;
        Vector2 mouseWorldPos = GetScreenToWorld2D(GetMousePosition(), m_view.camera);
        m_view.camera.offset = GetMousePosition();
...
void AstrolabeController::HandleInteraction(entt::registry& registry, entt::entity player, const TalentGraph& graph, const AstrolabeComponent* comp, uint32_t hoverId, const AstrolabeTalentNode* hoveredNode, const ProfessionStar* hoveredStar) {
    if (m_showVowDialog || m_alpha < 0.9f) return;
...
void AstrolabeController::DrawOverlay(const AstrolabeComponent* comp, float scale) {
    if (comp) {
        UISystem::DrawTextUI(TextFormat("可用星尘: %d", comp->available_points), 50, 50, 30, GOLD, m_alpha);
        UISystem::DrawTextUI(TextFormat("剑修亲和: %d", comp->getAffinity(ProfessionID::BladeAscendant)), 50, 100, 20, WHITE, m_alpha);
    }
    ...
    if (m_failMessageTimer > 0.0f) {
        m_failMessageTimer -= GetFrameTime();
...
void AstrolabeController::DrawTooltips(const TalentGraph& graph, const AstrolabeComponent* comp, uint32_t hoverId, const AstrolabeTalentNode* hoveredNode, const ProfessionStar* hoveredStar, float scale) {
...
void AstrolabeController::EmitEnergyFlow(const TalentGraph& graph, ProfessionID from, const AstrolabeTalentNode& to) {
...
void AstrolabeController::EmitSupernova(const AstrolabeTalentNode& node) {
...
void AstrolabeController::DrawVowDialog(entt::registry& registry, entt::entity player, const ProfessionStar& star) {
    ...
    if (hover && IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
        m_vowHoldProgress += dt / VOW_HOLD_DURATION;
        ...
        m_vowHoldProgress = std::max(0.0f, m_vowHoldProgress - dt * 2.0f);  // Fast decay
    }
}
