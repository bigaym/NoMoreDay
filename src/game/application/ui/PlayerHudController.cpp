...
PlayerHudController::PlayerHudController(UiRuntime& runtime)
    : m_runtime(runtime) {
  UiNodeDesc desc;
  desc.id = static_cast<UiId>(entt::hashed_string("ui_player_hud").value());
  desc.parent = kRootUiId;
  // Full-viewport HUD anchor. Draw is immediate-mode raylib (2K reference
...
void PlayerHudController::Update(entt::registry& registry) {
  m_hasPlayerData = false;
  ...
  (void)registry.try_get<SwordIntentComponent>(player);
  (void)registry.try_get<BladeMasteryComponent>(player);
  (void)registry.try_get<BladeResourceComponent>(player);
  m_hasPlayerData = true;
}

const char* PlayerHudController::ResolveBladeResourceLabel(
...
std::string PlayerHudController::ResolveBladeResourceDetailText(
...
std::string PlayerHudController::ResolveBladeResourceFeedbackText(
    const BladeMasteryComponent& mastery,
    const BladeResourceComponent& bladeResource) {
...
std::string PlayerHudController::ResolveBladeResourceRuntimeDetailText(
...
std::string PlayerHudController::ResolveBladeResourceRuntimeFeedbackText(
    const entt::registry& registry, const entt::entity player,
    const BladeMasteryComponent& mastery,
    const BladeResourceComponent& bladeResource, const CombatStats& stats) {
    (void)registry;
...
std::string PlayerHudController::ResolveSwordFlowFeedbackText(
    const BladeResourceComponent& bladeResource) {
    if (bladeResource.kind != BladeResourceKind::SwordFlow) {
        return {};
    }
...
void PlayerHudController::Draw(entt::registry& registry) {
    auto view = registry.view<PlayerTag, CombatStats>();
    ...
    // HP + Barrier Text
    std::string hpText = std::to_string(static_cast<int>(stats.health)) + " / " + std::to_string(static_cast<int>(stats.max_health));
    if (hasBarrier && barrierDisplayValue > 0.0f) {
        hpText += " (+" + std::to_string(static_cast<int>(barrierDisplayValue)) + ")";
    }
    ...
    // Mana Text
    std::string manaText = std::to_string(static_cast<int>(stats.mana)) + " / " + std::to_string(static_cast<int>(stats.max_mana));
