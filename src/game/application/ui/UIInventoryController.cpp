...
UIInventoryController::UIInventoryController(UiRuntime& runtime,
  ...
  // Equipment-slot hover animation state (U8: instance-owned, was
  // UISystem::State.equipmentSlotAnims; sized to the Draw loop bound).
  m_equipmentSlotAnims.resize(11);

  UiNodeDesc desc;
  desc.id = kUIInventoryRootNode;
  desc.parent = kRootUiId;
  // Full-viewport declarative anchor. The inventory panel itself is drawn by
  // the immediate-mode Draw below; this node is the host-owned root that the
  // runtime tracks for lifecycle purposes.
  ...
  desc.visible = true;
  // The panel is interactive, but input is routed through the immediate-mode
  // path; the declarative node must not intercept pointers.
...
void UIInventoryController::Update(entt::registry& registry,
  ...
  // U8: the visibility flag is instance state (m_visible); cross-layer writers
  // (host KEY_C/KEY_K, UIStashController::Open, sibling closes) go through the
  // host API, so no re-adopt is needed. Only the instance alpha animation
  // remains here.
...
void UIInventoryController::SetVisible(bool visible) {
  ...
  if (!visible) {
    // Legacy UIInventory::Toggle close path: reset the page and close any
    // context menu opened from the panel (routed through the hosted overlay).
    m_inventoryPage = 0;
    if (m_uiHost) {
...
void UIInventoryController::Toggle() {
  SetVisible(!m_visible);
}

void UIInventoryController::Draw(entt::registry& registry) {
  ...
  if (allowInventoryInput) {
    ...
    UIPanelDragService::UpdatePanelDrag(m_panelState, UIPanelID::Inventory,
  ...
  if (CheckCollisionPointRec(mousePos, {panelX, panelY, panelW, panelH})) {
    // U8: mouse-over-UI gate routes through the host instance member
    // (was UISystem::State.isMouseOverUI).
    if (m_uiHost) {
  ...
  // U8: the drag session is host-owned (single instance across panels); all
  // drag state reads/writes below route through it instead of the legacy
  // State fields.
  UIDragSession& drag = DragSession();

  auto handleNonInventoryEquipmentDrop = [&](EquipmentSlot targetSlot) -> bool {
    if (!InventorySystem::equipItem(registry, player, drag.draggedItem, targetSlot)) {
      return false;
    }

    if (drag.dragSourceEquipmentSlot != EquipmentSlot::None &&
        drag.dragSourceEquipmentSlot != targetSlot) {
      equip->set(drag.dragSourceEquipmentSlot, entt::null);
    }
  ...
  for (int i = 0; i < 11; ++i) {
    ...
    // Animation (U8: per-controller instance state, was
    // UISystem::State.equipmentSlotAnims).
    auto& anim = m_equipmentSlotAnims[i];
    ...
    // U8: route the panel hover write through the host channel (the tooltip
    // controller consumes the hover source; the host may be null in headless
    // unit tests). The legacy static hovered-item slot stays write-free here.
    if (isHovered && item != entt::null && drag.draggedItem == entt::null) {
      if (m_uiHost) {
    ...
    if (allowInventoryInput && isHovered && IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && IsKeyDown(KEY_LEFT_SHIFT) && item != entt::null) {
      if (!InventorySystem::unequipItem(registry, player, slotType)) {
        ...
        // controller); skipped when the host is null (headless tests).
        if (m_uiHost) {
          m_uiHost->ShowMessageBox("背包已满");
        }
      }
    }
      ...
      drag.dragSourceEquipmentSlot = slotType;
    }

    // U8: right-click context menu opens through the hosted overlay controller
    // (GameUiHost::OpenContextMenu -> OverlayController::OpenContextMenu); the
    // overlay re-adopts State each frame, so the menu shows at the original
    // UISystem::Draw overlay pass.
    if (allowInventoryInput && isHovered && IsMouseButtonPressed(MOUSE_RIGHT_BUTTON) && item != entt::null) {
      if (m_uiHost) {
        m_uiHost->OpenContextMenu(item, false, -1, slotType);
      }
    }

    // Socketing Logic (Drag Rune -> Equipment)
    bool handledDrop = false;
    if (allowInventoryInput && isHovered && IsMouseButtonReleased(MOUSE_LEFT_BUTTON) && drag.draggedItem != entt::null) {
       auto* dragItem = registry.try_get<ItemComponent>(drag.draggedItem);
       ...
       if (dragItem && targetItem && RunewordSystem::isRune(dragItem->id)) {
           ...
           if (freeSocketIdx != -1) {
               ...
               entt::entity runeToSocket = drag.draggedItem;
               bool wasSplit = false;
               ...
               if (CraftingSystem::socketRune(registry, item, runeToSocket, freeSocketIdx) == CraftingResult::Success) {
                   ...
                   if (!wasSplit) {
                        ...
                        if (drag.isDraggingFromInventory) {
                            inv->items[drag.dragSourceInventoryIndex] = entt::null;
                        } else if (drag.dragSourceBagSlotIndex != -1) {
                            InventorySystem::unequipBag(registry, player, drag.dragSourceBagSlotIndex, false); // Detach
                        }
                   }
               } else {
                   ...
                   if (m_uiHost) {
                     m_uiHost->ShowMessageBox("镶嵌失败");
                   }
               }
           } else {
               if (m_uiHost) {
                 m_uiHost->ShowMessageBox("没有可用插槽");
               }
    ...
    if (allowInventoryInput && !handledDrop && isHovered && IsMouseButtonReleased(MOUSE_LEFT_BUTTON) && drag.draggedItem != entt::null) {
      ...
      if (drag.isDraggingFromInventory && drag.dragSourceInventoryIndex != -1) {
        equipSucceeded = InventorySystem::swapInventoryItemIntoEquipment(
            registry, player, drag.dragSourceInventoryIndex, slotType);
      } else {
    ...
    UIRenderer::DrawSlot(font, registry, x - offset, y - offset, equipSlotSize * slotScale, (drag.draggedItem == item) ? entt::null : item, slotDefs[i].label, isHovered, false, alpha, slotType);
  ...
  if (m_activeTab == 0) {
    ...
    // U8: host-owned drag session (same pattern as the equipment slot pass).
    UIDragSession& drag = DragSession();
    ...
    for (int i = 0; i < renderCount; ++i) {
      ...
      // U8: hover write through the host channel (tooltip controller).
      if (isHovered && item != entt::null && drag.draggedItem == entt::null) {
        if (m_uiHost) {
          m_uiHost->SetHoveredItem(item);
        }
      ...
      if (allowInventoryInput && isHovered && IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && item != entt::null) {
        ...
        drag.dragSourceInventoryIndex = i;
        drag.dragSourceEquipmentSlot = EquipmentSlot::None;
        drag.dragSourceBagSlotIndex = -1;
      }
      
      // Right Click (U8: opens through the hosted overlay controller)
      if (allowInventoryInput && isHovered && IsMouseButtonPressed(MOUSE_RIGHT_BUTTON) && item != entt::null) {
        if (m_uiHost) {
          m_uiHost->OpenContextMenu(item, true, i, EquipmentSlot::None);
        }
      }
      ...
      if (allowInventoryInput && isHovered && IsMouseButtonReleased(MOUSE_LEFT_BUTTON) && drag.draggedItem != entt::null) {
         ...
         if (dragItem && targetItem && item != drag.draggedItem && RunewordSystem::isRune(dragItem->id)) {
              // Check available socket
             int freeSocketIdx = -1;
             if (targetItem->sockets.size() < (size_t)targetItem->socketCount) {
                 freeSocketIdx = (int)targetItem->sockets.size();
                 ...
                 if (CraftingSystem::socketRune(registry, item, runeToSocket, freeSocketIdx) == CraftingResult::Success) {
                     ...
                     if (!wasSplit) {
                          ...
                          if (drag.isDraggingFromInventory && drag.dragSourceInventoryIndex != -1) {
                              ...
                              if (equip && drag.dragSourceEquipmentSlot != EquipmentSlot::None) {
                                  equip->set(drag.dragSourceEquipmentSlot, entt::null);
                              } else if (drag.dragSourceBagSlotIndex != -1) {
                                  InventorySystem::unequipBag(registry, player, drag.dragSourceBagSlotIndex, true);
                              }
                          }
                     }
                 } else {
                     ...
                     // If we tried to socket and failed, we shouldn't try swap.
                     handledDropInv = true; 
                 }
             } else {
      ...
      if (allowInventoryInput && !handledDropInv && isHovered && IsMouseButtonReleased(MOUSE_LEFT_BUTTON) && drag.draggedItem != entt::null) {
         if (drag.isDraggingFromStash) {
              if (StashSystem::withdrawToSpecificSlot(registry, 
                   drag.dragSourceStashType, 
                   drag.dragSourceStashTab, 
                   drag.dragSourceStashSlot, 
                   player, i)) {
                   drag.draggedItem = entt::null;
              }
         } else if (drag.isDraggingFromInventory) {
              std::swap(inv->items[drag.dragSourceInventoryIndex], inv->items[i]);
         } else {
              if (equip && drag.dragSourceEquipmentSlot != EquipmentSlot::None) {
                  InventorySystem::moveEquippedItemToInventorySlot(registry, player, drag.dragSourceEquipmentSlot, i);
              } else if (drag.dragSourceBagSlotIndex != -1) {
                  InventorySystem::moveBagItemToInventorySlot(registry, player, drag.dragSourceBagSlotIndex, i);
              }
         }
         drag.draggedItem = entt::null;
      }

      UIRenderer::DrawSlot(font, registry, x, y, invSlotSize, (drag.draggedItem == item) ? entt::null : item, nullptr, isHovered, false, alpha);
    }
    ...
    if (bank) {
      ...
      // IsSearchFocused() for the typing gate (the legacy State.isTyping write
      // is gone; F2 removes the legacy field).
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    // U8: host-owned drag session (same pattern as the equipment/grid passes).
    UIDragSession& drag = DragSession();

    // U8: hover write through the host channel (tooltip controller).
    if (isHovered && bagItem != entt::null && drag.draggedItem == entt::null) {
      if (m_uiHost) {
        m_uiHost->SetHoveredItem(bagItem);
      }
    }

    // Drag Start from Bag Slot
    if (allowInventoryInput && isHovered && IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && bagItem != entt::null) {
      drag.draggedItem = bagItem;
      drag.isDraggingFromInventory = false;
    ...
    // Drop into Bag Slot
    if (allowInventoryInput && isHovered && IsMouseButtonReleased(MOUSE_LEFT_BUTTON) && drag.draggedItem != entt::null) {
      auto* dragItemComp = registry.try_get<ItemComponent>(drag.draggedItem);
      if (dragItemComp && dragItemComp->type == ItemType::Bag) {
        // 如果是从另一个背包槽位拖过来的，先解除旧位置的引用
        if (drag.dragSourceBagSlotIndex != -1 && drag.dragSourceBagSlotIndex != i) {
           // 调用 unequipBag 但不塞回物品栏，因为下一步要放入新槽位
           InventorySystem::unequipBag(registry, player, drag.dragSourceBagSlotIndex, false);
        }

        if (InventorySystem::equipBag(registry, player, drag.draggedItem, i)) {
          drag.draggedItem = entt::null;
        } else {
    ...
    UIRenderer::DrawSlot(font, registry, x, y, bagSlotSize, (drag.draggedItem == bagItem) ? entt::null : bagItem, "扩展", isHovered, false, alpha);
  }
}
