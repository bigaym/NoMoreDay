...
inline void to_json(nlohmann::json &j, const AffixType &e) {
  j = static_cast<uint16_t>(e);
}
inline void from_json(const nlohmann::json &j, AffixType &e) {
  e = static_cast<AffixType>(j.get<uint16_t>());
}
...
inline const char *GetAffixDescriptionRef(const Affix &affix, bool showTier) {
  ...
  switch (affix.type) {
  case AffixType::Strength:
    return TextFormat("%s+%.0f 力量", prefix, val);
  case AffixType::Dexterity:
    return TextFormat("%s+%.0f 敏捷", prefix, val);
  case AffixType::Intelligence:
    return TextFormat("%s+%.0f 智力", prefix, val);
  case AffixType::Vitality:
    return TextFormat("%s+%.0f 体质", prefix, val);
  case AffixType::AllAttributes:
    return TextFormat("%s+%.0f 所有属性", prefix, val);

  case AffixType::FlatHealth:
    return TextFormat("%s+%.0f 生命", prefix, val);
  case AffixType::PercentHealth:
    return TextFormat("%s+%.0f%% 生命", prefix, val);
  case AffixType::FlatMana:
    return TextFormat("%s+%.0f 法力", prefix, val);

  case AffixType::FlatPhysicalDamage:
    return TextFormat("%s+%.0f 物理伤害", prefix, val);
  case AffixType::FlatFireDamage:
    return TextFormat("%s+%.0f 火焰伤害", prefix, val);
  case AffixType::FlatColdDamage:
    return TextFormat("%s+%.0f 冰霜伤害", prefix, val);
  case AffixType::FlatLightningDamage:
    return TextFormat("%s+%.0f 闪电伤害", prefix, val);
  case AffixType::FlatPoisonDamage:
    return TextFormat("%s+%.0f 毒素伤害", prefix, val);
  case AffixType::FlatShadowDamage:
    return TextFormat("%s+%.0f 暗影伤害", prefix, val);
  ...
  case AffixType::CritChance:
    return TextFormat("%s+%.1f%% 暴击率", prefix, val);
  case AffixType::CritDamage:
    return TextFormat("%s+%.1f%% 暴击伤害", prefix, val);
  case AffixType::AttackSpeed:
    return TextFormat("%s+%.0f%% 攻击速度", prefix, val);
  case AffixType::CastSpeed:
    return TextFormat("%s+%.0f%% 施法速度", prefix, val);
  case AffixType::Accuracy:
    return TextFormat("%s+%.0f%% 命中率", prefix, val);

  case AffixType::FlatArmor:
    return TextFormat("%s+%.0f 护甲", prefix, val);
  case AffixType::PercentArmor:
    return TextFormat("%s+%.0f%% 护甲", prefix, val);
  case AffixType::ResistAll:
    return TextFormat("%s+%.0f%% 全抗性", prefix, val);
  case AffixType::ResistFire:
    return TextFormat("%s+%.0f%% 火焰抗性", prefix, val);
  case AffixType::ResistCold:
    return TextFormat("%s+%.0f%% 冰霜抗性", prefix, val);
  case AffixType::ResistLightning:
    return TextFormat("%s+%.0f%% 闪电抗性", prefix, val);
  case AffixType::ResistPoison:
    return TextFormat("%s+%.0f%% 毒素抗性", prefix, val);
  case AffixType::ResistShadow:
    return TextFormat("%s+%.0f%% 暗影抗性", prefix, val);

  case AffixType::Thorns:
    return TextFormat("%s+%.0f 荆棘伤害", prefix, val);
  case AffixType::DamageReduction:
    return TextFormat("%s+%.0f%% 伤害减免", prefix, val);

  case AffixType::HealthRegen:
    return TextFormat("%s+%.1f 生命回复", prefix, val);
  case AffixType::ManaRegen:
    return TextFormat("%s+%.1f 法力回复", prefix, val);
  case AffixType::PercentHealthRegen:
    return TextFormat("%s+%.0f%% 生命回复", prefix, val);
  case AffixType::PercentManaRegen:
    return TextFormat("%s+%.0f%% 法力回复", prefix, val);

  case AffixType::MoveSpeed:
    return TextFormat("%s+%.0f%% 移动速度", prefix, val);
  case AffixType::CooldownReduction:
    return TextFormat("%s+%.0f%% 冷却缩减", prefix, val);

  case AffixType::LifeSteal:
    return TextFormat("%s+%.1f%% 生命偷取", prefix, val);
  case AffixType::LifeOnHit:
    return TextFormat("%s+%.1f 击中回复", prefix, val);
  case AffixType::ManaOnHit:
    return TextFormat("%s+%.1f 击中回蓝", prefix, val);

  case AffixType::PlusAllSkills:
    return TextFormat("%s+%.0f 所有技能等级", prefix, val);
  case AffixType::PlusFlowingThrust:
    return TextFormat("%s+%.0f 流云刺等级", prefix, val);
  case AffixType::PlusRendingWave:
    return TextFormat("%s+%.0f 裂空斩等级", prefix, val);
  case AffixType::TitanGrip:
    return "泰坦之握 (可单手持双手武器)";

  case AffixType::FlatDodgeRating:
    return TextFormat("%s+%.0f 闪避评级", prefix, val);
  case AffixType::PercentDodgeRating:
    return TextFormat("%s+%.0f%% 闪避评级", prefix, val);
  case AffixType::FlatBlockRating:
  ...
  default: {
    if (IsLegendaryAffix(affix.type)) {
      ...
      if (lookup) {
        ...
        if (name)
          return TextFormat("%s%s", prefix, name);
      }
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(RuneComponent, weaponEffects, armorEffects,
                                   jewelryEffects)
