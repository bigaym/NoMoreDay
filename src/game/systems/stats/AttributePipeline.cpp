...
struct StatCalculation {
  ...
  float Result() const { return (base + flat) * (1.0f + percent_add) * percent_mult; }
};

static void ApplyStatCalculation(StatCalculation &c, ModifierMode mode, float value) {
  switch (mode) {
  case ModifierMode::Flat: c.flat += value; break;
  case ModifierMode::PercentAdd: c.percent_add += value / 100.0f; break;
  case ModifierMode::PercentMult: c.percent_mult *= (1.0f + value / 100.0f); break;
  }
}
...
static void ApplyStatModifier(std::array<StatCalculation, static_cast<size_t>(StatType::Count)> &calcs, StatType type, ModifierMode mode, float value) {
  if (type < StatType::Count) ApplyStatCalculation(calcs[static_cast<size_t>(type)], mode, value);
}
...
public:
  ...
  AffixDispatcher() {
    ...
    auto bind = [&](AffixType type, StatType stat, ModifierMode mode) {
      handlers[(int)type] = [stat, mode](AffixContext &ctx, const Affix &a) { ApplyStatModifier(ctx.calcs, stat, mode, a.value); };
    };
    ...
    handlers[(int)AffixType::AllAttributes] = [](AffixContext &ctx, const Affix &a) {
      for(auto s : {StatType::Strength, StatType::Dexterity, StatType::Intelligence, StatType::Vitality}) ApplyStatModifier(ctx.calcs, s, ModifierMode::Flat, a.value);
    };
    auto bindDirect = [&](AffixType type, DamageType dt) {
      handlers[(int)type] = [dt](AffixContext &ctx, const Affix &a) { ctx.combat.flat_damage[(int)dt] += a.value; };
    };
    ...
    bind(AffixType::PercentPhysicalDamage, StatType::PhysicalDamage, ModifierMode::PercentAdd);
    bind(AffixType::PercentFireDamage, StatType::FireDamage, ModifierMode::PercentAdd);
    bind(AffixType::PercentColdDamage, StatType::ColdDamage, ModifierMode::PercentAdd);
    bind(AffixType::PercentLightningDamage, StatType::LightningDamage, ModifierMode::PercentAdd);
    bind(AffixType::PercentPoisonDamage, StatType::PoisonDamage, ModifierMode::PercentAdd);
    bind(AffixType::PercentShadowDamage, StatType::ShadowDamage, ModifierMode::PercentAdd);
    ...
    bind(AffixType::AttackSpeed, StatType::AttackSpeed, ModifierMode::PercentAdd);
    ...
    bind(AffixType::PercentHealth, StatType::MaxHealth, ModifierMode::PercentAdd);
    ...
    bind(AffixType::ResistLightning, StatType::ResistLightning, ModifierMode::Flat);
    ...
    bind(AffixType::CooldownReduction, StatType::CooldownReduction, ModifierMode::Flat);
    ...
    bind(AffixType::PercentHealthRegen, StatType::HealthRegen, ModifierMode::PercentAdd);
    bind(AffixType::PercentManaRegen, StatType::ManaRegen, ModifierMode::PercentAdd);
    ...
    bind(AffixType::PercentDodgeRating, StatType::DodgeRating, ModifierMode::PercentAdd);
    bind(AffixType::FlatBlockRating, StatType::BlockRating, ModifierMode::Flat);
    bind(AffixType::PercentBlockRating, StatType::BlockRating, ModifierMode::PercentAdd);
    handlers[(int)AffixType::DamageReduction] = [](AffixContext &c, const Affix &a) { ApplyStatModifier(c.calcs, StatType::GlobalDamageReduction, ModifierMode::Flat, a.value); };
    handlers[(int)AffixType::LifeSteal] = [](AffixContext &c, const Affix &a) { ApplyStatModifier(c.calcs, StatType::LifeSteal, ModifierMode::Flat, a.value); };
    handlers[(int)AffixType::TitanGrip] = [](AffixContext &ctx, const Affix &) { ctx.hasTitanGrip = true; };
    handlers[(int)AffixType::PlusAllSkills] = [](AffixContext &ctx, const Affix &a) {
      if (auto *active = ctx.registry.try_get<ActiveSkillsComponent>(ctx.entity)) {
        for (auto &s : active->specialized_slots) if (s.skill_id != 0) s.bonus_levels += (int)a.value;
      }
    };
  }
  void Dispatch(AffixContext &ctx, const Affix &affix) const { if (static_cast<size_t>(affix.type) < TABLE_SIZE) handlers[static_cast<size_t>(affix.type)](ctx, affix); }
  static const AffixDispatcher &Get() { static AffixDispatcher instance; return instance; }
};

struct SetTrack { uint32_t hash; int count; const std::vector<SetBonus> *defs; };

void AttributePipeline::Calculate(entt::registry &registry, entt::entity entity) {
  ...
  if (auto *active = registry.try_get<ActiveSkillsComponent>(entity)) { for (auto &s : active->specialized_slots) s.bonus_levels = 0; }
  auto &global_mods = registry.get_or_emplace<GlobalModifierComponent>(entity);
  global_mods.modifiers.clear(); global_mods.stat_modifiers.clear();
  std::array<StatCalculation, static_cast<size_t>(StatType::Count)> calcs;
  using namespace NoMoreDay::Constants::Combat;
  ...
  for (int i = 0; i < 6; ++i) calcs[static_cast<size_t>(StatType::PhysicalDamage) + i].base = 100.0f;
  ...
  if (auto *enemy = registry.try_get<EnemyStateComponent>(entity)) {
    auto rarity = registry.all_of<EnemyRarityComponent>(entity) ? registry.get<EnemyRarityComponent>(entity).rarity : EnemyRarityComponent::NORMAL;
    ...
    auto applyRes = [&](Tag tag, StatType t) {
      float val = bonus; if (HasTag(raceData.resistances, tag)) val += NATIVE_RES;
      if (val > 0) ApplyStatModifier(calcs, t, ModifierMode::Flat, val);
    };
    applyRes(Tag::Physical, StatType::ResistPhysical); applyRes(Tag::Fire, StatType::ResistFire);
    applyRes(Tag::Cold, StatType::ResistCold); applyRes(Tag::Lightning, StatType::ResistLightning);
    applyRes(Tag::Poison, StatType::ResistPoison); applyRes(Tag::Shadow, StatType::ResistShadow);
  ...
  if (auto *primary = registry.try_get<PrimaryStats>(entity)) {
    calcs[static_cast<size_t>(StatType::Strength)].base = primary->strength; calcs[static_cast<size_t>(StatType::Dexterity)].base = primary->dexterity;
    calcs[static_cast<size_t>(StatType::Intelligence)].base = primary->intelligence; calcs[static_cast<size_t>(StatType::Vitality)].base = primary->vitality;
  }
  ...
  auto procAff = [&](const std::vector<Affix> &affs) {
    for (const auto &a : affs) {
      if (a.required_tags != Tag::None) {
        StatModifier m; m.type = static_cast<StatType>(a.type); m.mode = ModifierMode::PercentAdd; m.value = a.value; m.required_tags = a.required_tags; m.source = ModifierSource::Item;
        global_mods.stat_modifiers.push_back(m);
      } else { AffixDispatcher::Get().Dispatch(ctx, a); }
    }
  };

  Tag etags = Tag::None;
  if (auto *stance = registry.try_get<MovementStanceComponent>(entity)) { if (stance->stance == MovementStance::SwordRiding) etags = etags | Tag::SwordRiding; }
  static thread_local std::vector<SetTrack> s_set_scratch; s_set_scratch.clear();
  bool hasMain = false, hasOff = false, is2H = false; float mainAtk = 0, offAtk = 0;
    ...
    for (const auto &eItem : eq->slots) {
      if (registry.valid(eItem) && registry.all_of<ItemComponent>(eItem)) {
        ...
        if (item.type == ItemType::Weapon && item.attack > 0) {
          if (item.slot == EquipmentSlot::MainHand) { mainAtk = item.attack; hasMain = true; is2H = item.isTwoHanded; }
          else if (item.slot == EquipmentSlot::OffHand) { offAtk = item.attack; hasOff = true; }
        }
        procAff(item.implicits); procAff(item.affixes);
        if (item.defense > 0) ApplyStatModifier(calcs, StatType::Armor, ModifierMode::Flat, item.defense);
        for (auto r : item.sockets) {
          if (registry.valid(r) && registry.all_of<RuneComponent>(r)) {
            const auto &rune = registry.get<RuneComponent>(r);
            if (item.type == ItemType::Weapon) procAff(rune.weaponEffects);
            else if (item.slot == EquipmentSlot::Neck || item.slot == EquipmentSlot::Ring1 || item.slot == EquipmentSlot::Ring2) procAff(rune.jewelryEffects);
            else procAff(rune.armorEffects);
          }
        }
        if (item.rarity == Rarity::Set && item.setNameHash != 0) {
          bool found = false; for (auto &t : s_set_scratch) { if (t.hash == item.setNameHash) { t.count++; found = true; break; } }
          if (!found) s_set_scratch.push_back({item.setNameHash, 1, &item.setBonuses});
        }
      }
    }
  }
  for (const auto &t : s_set_scratch) { if (t.defs) { for (const auto &sb : *t.defs) { if (t.count >= sb.requiredCount) procAff(sb.bonuses); } } }

  if (hasMain) {
    if (hasOff) {
      float avg = (mainAtk + offAtk) * 0.5f; stats.min_weapon_damage = avg * 0.9f; stats.max_weapon_damage = avg * 1.1f;
      ApplyStatModifier(calcs, StatType::AttackSpeed, ModifierMode::PercentAdd, System::DUAL_WIELD_AS_BONUS);
    } else {
      stats.min_weapon_damage = mainAtk * 0.9f; stats.max_weapon_damage = mainAtk * 1.1f;
      if (is2H) { stats.min_weapon_damage *= System::TWO_HANDED_DMG_BONUS; stats.max_weapon_damage *= System::TWO_HANDED_DMG_BONUS; }
    }
    if (stats.knockback < 0.1f) stats.knockback = 20.0f;
  } else if (registry.all_of<EquipmentComponent>(entity)) {
    stats.min_weapon_damage = 2.0f; stats.max_weapon_damage = 3.0f; stats.knockback = 10.0f;
  } else if (auto *wc = registry.try_get<WeaponComponent>(entity)) {
    stats.min_weapon_damage = wc->damage; stats.max_weapon_damage = wc->damage;
  } else if (auto *en = registry.try_get<EnemyStateComponent>(entity)) {
    float bd = kRaceData[static_cast<size_t>(en->raceType)].baseDamage * (1.0f + en->level * 0.1f);
    stats.min_weapon_damage = bd * 0.9f * mapDmgMult; stats.max_weapon_damage = bd * 1.1f * mapDmgMult;
  }

  if (auto *se = registry.try_get<SoulEaterComponent>(entity)) {
    if (se->stacks > 0) {
      float db = se->stacks * se->damagePerStack; float ab = se->stacks * se->attackSpeedPerStack;
      for (int i = 0; i < 6; ++i) ApplyStatModifier(calcs, static_cast<StatType>(static_cast<int>(StatType::PhysicalDamage) + i), ModifierMode::PercentAdd, db);
      ApplyStatModifier(calcs, StatType::AttackSpeed, ModifierMode::PercentAdd, ab);
    }
  }
  ...
  if (auto *list = registry.try_get<ModifierList>(entity)) {
    for (const auto &m : list->modifiers) { if (m.required_tags == Tag::None || HasTag(etags, m.required_tags)) ApplyStatModifier(calcs, m.type, m.mode, m.value); }
  }
  if (auto *ac = registry.try_get<MonsterAffixComponent>(entity)) {
    for (auto at : ac->affixes) {
      ...
      for (int i = 0; i < def.statModCount; ++i) ApplyStatModifier(calcs, def.statMods[i].type, def.statMods[i].mode, def.statMods[i].value);
      if (ac->isBerserk) { stats.min_weapon_damage *= 2.0f; stats.max_weapon_damage *= 2.0f; }
    }
  }
  if (auto *act = registry.try_get<ActiveSkillsComponent>(entity)) {
    for (const auto &s : act->specialized_slots) {
      if (s.skill_id == 0) continue;
      const auto *tr = SkillRegistry::Get().GetSkillTree(s.skill_id); if (!tr) continue;
      for (const auto &[nid, pts] : s.allocated_points) {
        if (pts <= 0) continue;
        auto nit = tr->nodes.find(nid); if (nit == tr->nodes.end()) continue;
        for (const auto &m : nit->second.stat_modifiers) { if (m.required_tags == Tag::None) ApplyStatModifier(calcs, m.type, m.mode, m.value * static_cast<float>(pts)); }
      }
    }
  }
  if (auto *as = registry.try_get<AstrolabeComponent>(entity)) {
    for (uint32_t nid : as->activated_nodes) {
      if (const auto *n = AstrolabeRegistry::Get().GetNode(nid)) { for (const auto &m : n->modifiers) { if (m.required_tags == Tag::None) ApplyStatModifier(calcs, m.type, m.mode, m.value); } }
    }
  }
  ...
  stats.effective_strength = str; stats.effective_dexterity = dex; stats.effective_intelligence = intel; stats.effective_vitality = vit;
  ...
  stats.health_regen += vit * Attribute::VIT_TO_HEALTH_REGEN; stats.mana_regen += intel * Attribute::INT_TO_MANA_REGEN;
  ...
  int al = (registry.all_of<EnemyStateComponent>(entity)) ? registry.get<EnemyStateComponent>(entity).level : 1;
  if (auto *p = registry.try_get<PlayerStats>(entity)) al = std::max(1, p->level);
  ...
  stats.max_barrier += calcs[static_cast<size_t>(StatType::MaxBarrier)].Result();
  stats.barrier_regen += calcs[static_cast<size_t>(StatType::BarrierRegen)].Result();
  stats.barrier_decay = calcs[static_cast<size_t>(StatType::BarrierDecay)].Result() / 100.0f;
  stats.barrier_delay = calcs[static_cast<size_t>(StatType::BarrierDelay)].Result();
  stats.barrier_retention = calcs[static_cast<size_t>(StatType::BarrierRetention)].Result() / 100.0f + intel * Attribute::INT_TO_BARRIER_RETENTION / 100.0f;
  stats.dodge_rating = calcs[static_cast<size_t>(StatType::DodgeRating)].Result();
  stats.block_rating = calcs[static_cast<size_t>(StatType::BlockRating)].Result();
  stats.effective_dodge = CombatFormula::CalculateDodgeChance(stats.dodge_rating, al) + calcs[static_cast<size_t>(StatType::DodgeChance)].Result();
  stats.dodge_chance = std::min(stats.effective_dodge, Scaling::DODGE_MAX_CHANCE);
  stats.effective_dodge = stats.dodge_chance;
  stats.block_chance = std::min(calcs[static_cast<size_t>(StatType::BlockChance)].Result(), Scaling::BLOCK_MAX_CHANCE);
  stats.block_amount = stats.block_rating;
  stats.effective_block_eff = CombatFormula::CalculateBlockEffectiveness(stats.block_rating, al);
  stats.effective_armor_dr = 1.0f - CombatFormula::CalculateArmorMultiplier(stats.armor, al);
  stats.damage_reduction = calcs[static_cast<size_t>(StatType::GlobalDamageReduction)].Result() / 100.0f;
  if (hasTitanGrip) registry.emplace<TitanGripTrait>(entity);
  if (auto *hp = registry.try_get<HealthComponent>(entity)) { hp->max = stats.max_health; if (hp->current > hp->max) hp->current = hp->max; stats.health = hp->current; }
  if (stats.max_barrier > 0 || stats.barrier > 0) (void)registry.get_or_emplace<BarrierComponent>(entity);
  stats.move_speed = std::min(calcs[static_cast<size_t>(StatType::MoveSpeed)].Result(), MOVE_SPEED_CAP);
  stats.raw_move_speed = calcs[static_cast<size_t>(StatType::MoveSpeed)].Result();
  stats.crit_chance = std::min(calcs[static_cast<size_t>(StatType::CritChance)].Result() / 100.0f, Cap::CRIT_CHANCE);
  stats.crit_damage = calcs[static_cast<size_t>(StatType::CritDamage)].Result() / 100.0f;
  stats.attack_speed = std::min(calcs[static_cast<size_t>(StatType::AttackSpeed)].Result() / 100.0f, Cap::ATTACK_SPEED);
  for (int i = 0; i < 6; ++i) { auto &c = calcs[static_cast<size_t>(StatType::PhysicalDamage) + i]; stats.damage_multipliers[i] = c.Result() / 100.0f; stats.damage_percent_add[i] = c.percent_add; stats.damage_percent_mult_component[i] = c.percent_mult; }
  float rall = calcs[static_cast<size_t>(StatType::ResistAll)].Result();
  for (int i = 0; i < 6; ++i) { float fr = (calcs[static_cast<size_t>(StatType::ResistPhysical) + i].Result() + rall) / 100.0f; stats.raw_resistances[i] = fr; stats.resistances[i] = std::min(fr, Cap::RESISTANCE); }
}

void AttributePipeline::ToGPU(const CombatStats &src, NoMoreDay::components::GPUVisualStats &dst) {
  dst.weaponDamage = src.effective_strength; dst.attackSpeed = src.attack_speed; dst.critChance = src.crit_chance; dst.critDamage = src.crit_damage; dst.defenseRating = src.effective_armor_dr;
  float br = (src.max_health > 0) ? (src.barrier / src.max_health) : 0.0f; dst.statusStrength = 0.0f; dst.glowIntensity = br;
  uint32_t r = 255, g = 255, b = 255, a = 255; dst.glowColorPacked = (a << 24) | (b << 16) | (g << 8) | r;
}
