...
void UICharacter::DrawStatRow(const char *label, const char *value, float x,
                              float &y, float width, float fontSize,
                              float alpha) {
...
// 辅助函数：绘制带上限检查的属性行
static void DrawCappedStatRow(const char *label, float currentVal, float cap,
...
void UICharacter::Draw(entt::registry &registry) {
  ...
  // 锚定左侧居中 (Center Left Anchor) - Initial values, overridden by
  // UpdatePanelDrag
  float panelX = margin;
  ...
  if (CheckCollisionPointRec(mousePos, viewRect)) {
    ...
    if (wheel != 0)
      s_charPanelScroll += wheel * 30.0f;
  }
  if (s_charPanelScroll > 0)
    s_charPanelScroll = 0;
  if (s_lastContentHeight > viewRect.height) {
    float minScroll = viewRect.height - s_lastContentHeight;
    if (s_charPanelScroll < minScroll)
      s_charPanelScroll = minScroll;
  } else {
    s_charPanelScroll = 0;
  }
