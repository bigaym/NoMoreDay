...
// Forward declaration
static void CreateToxicPool(entt::registry& registry, const Position& pos, entt::entity owner);

// Helper to convert DamageType to Tag
static Tag DamageTypeToTag(DamageType type) {
    switch (type) {
        case DamageType::Physical: return Tag::Physical;
        case DamageType::Fire: return Tag::Fire;
        case DamageType::Cold: return Tag::Cold;
        case DamageType::Lightning: return Tag::Lightning;
        case DamageType::Poison: return Tag::Poison;
        case DamageType::Shadow: return Tag::Shadow;
        default: return Tag::None;
    }
}

void HazardSystem::Update(entt::registry& registry, float dt, const systems::SpatialHashGrid& grid) {
...
void HazardSystem::ProcessHazards(entt::registry& registry, float dt, const systems::SpatialHashGrid& grid) {
...
void HazardSystem::ProcessFrozenOrbs(entt::registry& registry, float dt, const systems::SpatialHashGrid& grid) {
    auto view = registry.view<FrozenOrbComponent, Position, Velocity, FrozenOrbTag>();
    std::vector<entt::entity> toDestroy;
        ...
        if (orb.isTraveling) {
            ...
            if (orb.currentTimer >= orb.stopDuration) {
                ...
                if (hazard) {
                    ...
                    grid.query(pos, 80.0f, [&](entt::entity target, const Position& targetPos) {
                    ...
                    for (int i = 0; i < 40; i++) {
                        ...
                        p.position = {pos.x, pos.y};
                        float angle = NoMoreDay::utils::ThreadSafeRandom::GetFloat(0.0f, 360.0f) * DEG2RAD;
                        float speed = NoMoreDay::utils::ThreadSafeRandom::GetFloat(100.0f, 350.0f);
                        p.velocity = {cosf(angle) * speed, sinf(angle) * speed};
                        p.color = {150, 220, 255, 220};
                        p.lifetime = NoMoreDay::utils::ThreadSafeRandom::GetFloat(0.6f, 1.0f);
                        p.maxLifetime = p.lifetime;
...
void HazardSystem::ProcessVolatileOrbs(entt::registry& registry, float dt) {
    auto view = registry.view<VolatileOrbComponent, Position, Velocity, VolatileOrbTag>();
    std::vector<entt::entity> toDestroy;
...
void HazardSystem::ProcessLightningGhosts(entt::registry& registry, float dt, const systems::SpatialHashGrid& grid) {
    auto view = registry.view<LightningGhostComponent, Position, LightningGhostTag>();
    std::vector<entt::entity> toDestroy;
    ...
    for (auto entity : view) {
        ...
        if (ghost.currentTimer >= ghost.explosionDelay) {
            ...
            DealAreaDamage(registry, pos, 50.0f, 80.0f, DamageType::Lightning, true, false, grid, owner);
            ...
            for (int i = 0; i < 25; i++) {
                ...
                p.position = {pos.x, pos.y};
                float angle = NoMoreDay::utils::ThreadSafeRandom::GetFloat(0.0f, 360.0f) * DEG2RAD;
                float speed = NoMoreDay::utils::ThreadSafeRandom::GetFloat(80.0f, 260.0f);
                p.velocity = {cosf(angle) * speed, sinf(angle) * speed};
                p.color = {255, 255, 100, 220};
...
void HazardSystem::ProcessVoidZones(entt::registry& registry, float dt) {
    auto view = registry.view<VoidZoneTag, HazardComponent, HazardVisualComponent>();
    ...
    for (auto entity : view) {
        ...
        if (visual.particleEmitTimer >= visual.particleEmitInterval) {
            ...
            for (int i = 0; i < visual.particlesPerEmit; i++) {
                components::GPUParticle p;
                p.position = {pos.x + NoMoreDay::utils::ThreadSafeRandom::GetFloat(-20.0f, 20.0f), 
                             pos.y + NoMoreDay::utils::ThreadSafeRandom::GetFloat(-20.0f, 20.0f)};
                p.velocity = {NoMoreDay::utils::ThreadSafeRandom::GetFloat(-10.0f, 10.0f), 
                ...
                p.maxLifetime = p.lifetime;
                p.scale = 2.5f * visual.visualScale + NoMoreDay::utils::ThreadSafeRandom::GetFloat(0.0f, 1.5f);
                systems::GPUParticleSystem::Get().Emit(p);
            }
        }
    }
...
void HazardSystem::ApplyChillDebuff(entt::registry& registry, entt::entity target, float duration, float slowAmount) {
    ...
    chill.modifiers.push_back({StatType::MoveSpeed, ModifierMode::PercentAdd, -slowAmount * 100.0f});
    
    effects.AddOrRefresh(chill);
}

void HazardSystem::DealAreaDamage(entt::registry& registry, Position center, float radius, 
                                  float damage, DamageType type, bool hitsPlayers, bool hitsEnemies,
                                  const systems::SpatialHashGrid& grid, entt::entity owner) {
    ...
    grid.query(center, radius, [&](entt::entity target, const Position& targetPos) {
        ...
        bool isEnemy = registry.all_of<EnemyTag>(target) && !registry.all_of<KilledTag>(target);
        
        if ((hitsPlayers && isPlayer) || (hitsEnemies && isEnemy)) {
            auto result = DamagePipeline::Calculate(registry, owner, target, 0, base_pool, Tag::None);
            
            // 修复：必须实际应用伤害
            CombatSystem::ApplyDamage(registry, target, result.total_damage, owner, result.is_crit, false);
            
            Tag damageTag = DamageTypeToTag(type);
            systems::EffectSystem::EmitDamagePopup(registry, {targetPos.x, targetPos.y - 20.0f}, 
                                                   result.total_damage, result.is_crit, damageTag);
        }
    });
}
