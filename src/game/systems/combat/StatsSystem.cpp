...
void StatsSystem::Recalculate(entt::registry &registry, entt::entity entity) {
  ...
  std::array<StatCalculation, static_cast<size_t>(StatType::Count)>
      calcs; // 初始化计算数组
  ...
  if (auto *enemy = registry.try_get<EnemyStateComponent>(entity)) {
    ...
    // Speed is currently not scaled by level in spec (or is it?), let's stick
    // to raceData for speed or add it to scaling if needed. Spec says nothing
    // about speed scaling. Use raceData.
    ...
    auto applyResist = [&](Tag tag, StatType type) {
      ...
      if (HasTag(raceData.resistances, tag))
        val += NATIVE_RESISTANCE_VALUE;
      // Add Scaling Bonus
      val +=
          scaled.resistanceBonus * 100.0f; // scaled is 0.0-0.75, stat is 0-100?
      ...
      // Need to confirm unit. MonsterScaling.cpp line 56:
      // result.resistanceBonus = overLevel * resPerLevel; resPerLevel is 0.002.
      // So for 10 levels, 0.02. So yes, multiply by 100 for StatsSystem if it
      // expects percentage points (0-100). Looking at ApplyStatModifier
      // calls... NATIVE_RESISTANCE_VALUE = 50.0f. ResistFire is Flat.
      ...
      // Check `StatsSystem.cpp` around line 64:
      // `combat.resistances.fill(0.0f);` Affixes: `Bind<StatType::ResistFire,
      // ModifierMode::Flat>(AffixType::ResistFire);` Affix definitions usually
      // use whole numbers (e.g. 10 for 10%). Let's assume StatsSystem working
      // with Percentage Points (0-100) and converts later, OR works with
      // 0.0-1.0. NATIVE_RESISTANCE_VALUE = 50.0f.  If 1.0 was 100%, 50 would be
      // god mode. So likely StatsSystem expects 0-100 logic or converts it.
      ...
      if (val > 0)
        ApplyStatModifier(calcs, type, ModifierMode::Flat, val);
    };
    ...
    // We apply the Bonus uniformly to ALL resists? Or just the ones the monster
    // has? Spec: "3.1 Constants (Common.hpp) ... Lv 1-100: Use race base ... Lv
    // 100+: bonusRes = ..." Implicitly, monsters usually gain ALL res or
    // specific res? "finalRes = min(0.75, baseRes + bonusRes)". This implies
    // bonus applies to whatever base is. If base is 0, final is bonusRes. So
    // yes, all elements get the bonus.

    float bonus = scaled.resistanceBonus * 100.0f;

    ApplyStatModifier(calcs, StatType::ResistPhysical, ModifierMode::Flat,
                      (HasTag(raceData.resistances, Tag::Physical)
                           ? NATIVE_RESISTANCE_VALUE
                           : 0.0f) +
                          bonus);
    ApplyStatModifier(calcs, StatType::ResistFire, ModifierMode::Flat,
                      (HasTag(raceData.resistances, Tag::Fire)
                           ? NATIVE_RESISTANCE_VALUE
                           : 0.0f) +
                          bonus);
    ApplyStatModifier(calcs, StatType::ResistCold, ModifierMode::Flat,
                      (HasTag(raceData.resistances, Tag::Cold)
                           ? NATIVE_RESISTANCE_VALUE
                           : 0.0f) +
                          bonus);
    ApplyStatModifier(calcs, StatType::ResistLightning, ModifierMode::Flat,
                      (HasTag(raceData.resistances, Tag::Lightning)
                           ? NATIVE_RESISTANCE_VALUE
                           : 0.0f) +
                          bonus);
    ApplyStatModifier(calcs, StatType::ResistPoison, ModifierMode::Flat,
                      (HasTag(raceData.resistances, Tag::Poison)
                           ? NATIVE_RESISTANCE_VALUE
                           : 0.0f) +
                          bonus);
    ApplyStatModifier(calcs, StatType::ResistShadow, ModifierMode::Flat,
                      (HasTag(raceData.resistances, Tag::Shadow)
                           ? NATIVE_RESISTANCE_VALUE
                           : 0.0f) +
                          bonus);
  }
  ...
  if (auto *stanceComp = registry.try_get<MovementStanceComponent>(entity)) {
    if (stanceComp->stance == MovementStance::SwordRiding) {
      ...
      LOG_DEBUG("StatsSystem: Recalculating with SwordRiding tag for entity {}",
                (uint32_t)entity);
    }
  }
  ...
  auto apply_conversions = [&](const std::vector<StatConversion> &convs) {
    for (const auto &conv : convs) {
      if (conv.source < StatType::Count && conv.target < StatType::Count) {
        if (conv.required_tags == Tag::None ||
            HasTag(entity_base_tags, conv.required_tags)) {
          float sourceVal = calcs[static_cast<size_t>(conv.source)].Result();
          ApplyStatModifier(calcs, conv.target, ModifierMode::Flat,
                            sourceVal * conv.ratio);
        }
      }
    }
  };
    ...
    for (const auto &affix : affixes) {
      ...
      if (affix.required_tags != Tag::None) {
        ...
        mod.type = static_cast<StatType>(
            affix.type); // Direct cast works for most common types
  ...
  if (registry.all_of<EquipmentComponent>(entity)) {
    ...
    offHandIsEmpty =
        !registry.valid(equipment.slots[(int)EquipmentSlot::OffHand]);
      ...
      if (registry.valid(itemEntity) &&
        ...
        for (auto runeEntity : item.sockets) {
          if (registry.valid(runeEntity) &&
              registry.all_of<RuneComponent>(runeEntity)) {
  ...
  if (hasMainHandWeapon) {
    ...
    if (registry.all_of<EquipmentComponent>(entity)) {
      using namespace NoMoreDay::Constants::Combat::
          System; // Wait, I should add these to System if needed
      // Actually I didn't add UNARMED specifically, but I can add them or use
      // defaults
  ...
  using namespace NoMoreDay::Constants::Combat;
  if (hasMainHandWeapon && combat.knockback < 0.1f)
    combat.knockback = 20.0f; // DEFAULT_WEAPON_KNOCKBACK
  ...
  if (registry.all_of<ModifierList>(entity)) {
    ...
    for (const auto &mod : list.modifiers) {
      if (mod.required_tags == Tag::None ||
          HasTag(entity_base_tags, mod.required_tags)) {
  ...
  if (auto *soulEater = registry.try_get<SoulEaterComponent>(entity)) {
    if (soulEater->stacks > 0) {
      ...
      for (int i = 0; i < 6; ++i) {
        ApplyStatModifier(
            ...
            static_cast<StatType>(
                static_cast<size_t>(StatType::PhysicalDamage) + i),
      ...
      LOG_DEBUG("StatsSystem: Applied Soul Eater bonuses (Stacks: {}, Dmg: "
                "+{}%, AS: +{}%)",
  ...
  if (registry.all_of<SwordHeartComponent>(entity)) {
    ...
    bool isSwordHeartActive =
        hasMainHandWeapon && offHandIsEmpty && !isTwoHanded;
    ...
    if (isSwordHeartActive) {
      ...
      float phys_inc =
          calcs[static_cast<size_t>(StatType::PhysicalDamage)].percent_add;
      float spell_bonus = phys_inc * SWORD_HEART_SPELL_BONUS_RATIO;

      for (int i = 1; i < 6; ++i) {
        calcs[static_cast<size_t>(StatType::PhysicalDamage) + i].percent_add +=
            spell_bonus;
      }

      LOG_INFO("Sword Heart ACTIVE for entity {}: +15% Weapon Dmg, +20% Block "
               "(50 Amt), +{:.1f}% Spell Dmg",
               (uint32_t)entity, spell_bonus * 100.0f);
    } else {
      LOG_DEBUG("Sword Heart INACTIVE for entity {}: Condition (One-handed "
                "Sword + Empty Offhand) not met.",
                (uint32_t)entity);
    }
  }
      ...
      if (const auto *node = reg.GetNode(node_id)) {
        ...
        for (const auto &effect : node->effects) {
          if (effect.value.find("IntToCritMult:") == 0) {
            ...
            apply_conversions(
                {{StatType::Intelligence, StatType::CritDamage, ratio}});
          } else if (effect.value.find("IntToArmor:") == 0) {
            float ratio = std::stof(effect.value.substr(11));
            apply_conversions(
                {{StatType::Intelligence, StatType::Armor, ratio}});
          }
        }
      }
    }
  ...
  if (auto *intent = registry.try_get<SwordIntentComponent>(entity)) {
    if (intent->stacks > 0) {
      ...
      ApplyStatModifier(calcs, StatType::AttackSpeed, ModifierMode::Flat,
                        (float)intent->stacks);
      LOG_TRACE(
          "StatsSystem: Applied {}% Attack Speed from {} Sword Intent stacks",
          intent->stacks, intent->stacks);
    }
  }
  ...
  combat.cast_speed =
      calcs[static_cast<size_t>(StatType::CastSpeed)].Result() / 100.0f;
  combat.accuracy =
      calcs[static_cast<size_t>(StatType::Accuracy)].Result() / 100.0f;
  combat.mana_on_hit = calcs[static_cast<size_t>(StatType::ManaOnHit)].Result();

  float finalCDR =
      calcs[static_cast<size_t>(StatType::CooldownReduction)].Result() / 100.0f;
  combat.raw_cooldown_reduction = finalCDR;
  combat.cooldown_reduction = std::min(finalCDR, Cap::CDR);

  combat.resource_cost_reduction =
      calcs[static_cast<size_t>(StatType::ResourceCostReduction)].Result() /
      100.0f;

  combat.projectile_speed =
      calcs[static_cast<size_t>(StatType::ProjectileSpeed)].Result() / 100.0f;
  combat.duration_scale =
      calcs[static_cast<size_t>(StatType::DurationScale)].Result() / 100.0f;
  combat.area_scale =
      calcs[static_cast<size_t>(StatType::AreaScale)].Result() / 100.0f;

  float finalDodge =
      calcs[static_cast<size_t>(StatType::DodgeChance)].Result() / 100.0f;
  combat.raw_dodge_chance = finalDodge;
  combat.dodge_chance = std::min(finalDodge, Cap::DODGE);

  float finalBlock =
      (combat.block_chance +
       calcs[static_cast<size_t>(StatType::BlockChance)].Result() / 100.0f);
  combat.raw_block_chance = finalBlock;
  combat.block_chance = std::min(finalBlock, Cap::BLOCK);

  combat.life_steal +=
      calcs[static_cast<size_t>(StatType::LifeSteal)].Result() / 100.0f;
  combat.life_on_hit +=
      calcs[static_cast<size_t>(StatType::LifeOnHit)].Result();
  combat.health_regen +=
      calcs[static_cast<size_t>(StatType::HealthRegen)].Result() - 1.0f;
  combat.mana_regen +=
      calcs[static_cast<size_t>(StatType::ManaRegen)].Result() - 1.0f;
  ...
  LOG_TRACE("StatsSystem: Recalculated for entity {}. Dmg: {:.1f}-{:.1f}, Str: "
            "{:.1f}, HP: {:.1f}",
            (uint32_t)entity, combat.min_weapon_damage,
            combat.max_weapon_damage, str, combat.max_health);
