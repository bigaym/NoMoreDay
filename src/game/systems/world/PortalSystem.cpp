    ...
    for (auto player : players) {
        ...
        for (auto portal : portals) {
            ...
            if (distSq < 20.0f * 20.0f) {
                ...
                m_sceneManager.SetOriginInfo(
                    m_sceneManager.GetCurrentBiome(),
...
void PortalSystem::UpdateTownPortalCasting(entt::registry& registry, float dt) {
    ...
    for (auto entity : view) {
        ...
        for (int i = 0; i < particleCount; ++i) {
            float angle = casting.elapsedTime * 8.0f + i * (2.0f * PI / particleCount);
            float radius = 15.0f + progress * 15.0f;
            
            NoMoreDay::components::GPUParticle p;
            p.position = {casting.castX + cosf(angle) * radius, casting.castY + sinf(angle) * radius};
...
void PortalSystem::SpawnTownPortal(entt::registry& registry, entt::entity caster) {
    auto* pos = registry.try_get<Position>(caster);
    if (!pos) return;
    
    auto portal = registry.create();
    registry.emplace<LocalLevelTag>(portal);
    registry.emplace<Position>(portal, pos->x, pos->y + 40.0f); // Slightly in front of player
    ...
    // Spawn visual effect
    auto& particleSys = NoMoreDay::systems::GPUParticleSystem::Get();
    auto splash = NoMoreDay::systems::InkEffectHelper::CreateInkSplash({pos->x, pos->y + 40.0f}, 20, 15.0f, 120.0f);
    for (auto& p : splash) {
        p.color = GOLD;
        particleSys.Emit(p);
    }
...
void PortalSystem::StartTownPortalCast(entt::registry& registry, entt::entity caster) {
...
void PortalSystem::CancelTownPortalCast(entt::registry& registry, entt::entity caster) {
...
void PortalSystem::Render(entt::registry& registry, const Camera2D& camera) {
    ...
    for (auto entity : view) {
        ...
        // 1. Outer glow ring
        DrawRing({pos.x, pos.y}, radius - 5.0f, radius, 0, 360, 32, ColorAlpha(ringColor, 0.4f));
        DrawRing({pos.x, pos.y}, radius - 8.0f, radius - 5.0f, 0, 360, 32, ColorAlpha(ringColor, 0.6f));
        ...
        for (int i = 0; i < numLines; ++i) {
            ...
            Vector2 start = {pos.x + cosf(angle) * innerRadius, pos.y + sinf(angle) * innerRadius};
            Vector2 end = {pos.x + cosf(angle) * outerRadius, pos.y + sinf(angle) * outerRadius};
        ...
        DrawCircle((int)pos.x, (int)pos.y, 6.0f * pulse, ColorAlpha(ringColor, 0.8f));
        ...
        if (fmodf(portal.animationTimer, 0.1f) < 0.016f) {
            ...
            for (int i = 0; i < 2; ++i) {
                float a = pAngle + i * PI;
                NoMoreDay::components::GPUParticle p;
                p.position = {pos.x + cosf(a) * (radius - 10.0f), pos.y + sinf(a) * (radius - 10.0f)};
    ...
    for (auto entity : castingView) {
        ...
        DrawRing({casting.castX, casting.castY}, 18.0f, 22.0f, 0, degrees, 32, ColorAlpha(GOLD, 0.7f));
        DrawRing({casting.castX, casting.castY}, 22.0f, 25.0f, 0, 360, 32, ColorAlpha(WHITE, 0.3f));
    }
}
