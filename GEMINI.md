...
* **指令运行**: 强制使用 **Windows PowerShell** 环境指令。
    * **分隔符**: 严禁使用 `&&`，必须使用 `;` 连接多条指令。
    * **原生命令**: 严禁使用 Linux/Unix 命令（如 `ls`, `rm`, `grep`, `cp`, `mv`）。必须使用对应的 PowerShell 指令：
        * 查找文件: `dir` 或 `Get-ChildItem`
        * 删除: `del` 或 `Remove-Item`
        * 文本搜索: `Select-String`
        * 复制/移动: `copy`/`move`
    * **环境**: 涉及 Python 脚本执行时，必须先尝试 `conda activate ai`。

## 4. 环境与资源

* **构建**: `.\build.bat`。
* **测试**: `./bin/NoMoreDayTests.exe`。
