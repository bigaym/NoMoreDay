...
TEST_CASE("Blade Formation Deep Dive") {
  ...
  SUBCASE("Standard Mode - Multiple Swords") {
    ...
    exec.skill_id = 3;
    exec.owner = player;
    // Manual bitset population for test
    for (auto const &[node, pts] :
         active.specialized_slots[0].allocated_points) {
      if (pts > 0)
        exec.active_nodes.set(node % 100);
    }
  ...
  SUBCASE("Giant Sword Mode - Merging") {
    // Unlock Talent 330 (Giant Sword)
    active.specialized_slots[0].allocated_points[330] = 1;
    active.specialized_slots[0].allocated_points[300] =
        5; // Even with extra swords, should stay 1
  ...
  SUBCASE("Giant Sword - Damage Scaling") {
    ...
    SkillSystem::AddPreCastHook(
        [&](entt::registry &r, entt::entity e, SkillExecution &ex) {
    ...
    SkillSystem::UpdateStates(
        registry, 0.1f); // Process Preparing -> Casting (Hook triggers here)
  ...
  SUBCASE("Standard Sword - Damage Scaling") {
    ...
    CHECK(damage_mult ==
          doctest::Approx(0.5f)); // Standard sword should be 0.5x
  }
}
