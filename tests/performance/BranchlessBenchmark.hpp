...
TEST_CASE("Performance: Branchless Logic Benchmark") {
  ...
  printf("  Branching: %lld us (Result: %.2f)\n", duration_branch,
         result_branch);
  printf("  SelectF:   %lld us (Result: %.2f) (%.2fx speedup)\n",
         duration_select, result_select,
         (float)duration_branch / duration_select);

  // Check equality to ensure logic is identical
  CHECK(result_branch == doctest::Approx(result_select));
}
