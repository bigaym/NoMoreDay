...
> **依赖:** 无（B 是 C/D 的前置地基）
> **状态:** [~] 进行中（B1/B5/B6/B7/B9-B12/B13 诊断完成；B8 合同级真实 GL binding fixture 已通过，但生产视觉回归证据仍缺失；B2/B3 真实 ShadowBuild 逐 pass GL gate 已通过（合同级，2026-08-04）；shader 双重释放 owner blocker 已修复（2026-08-04）；B4 真实 LightCulling 逐 pass GL gate 已通过（合同级，2026-08-05）且生产 Setup observer 接线完成；**B2-B4 生产手工绑定删除已完成（2026-08-06，见 B2-B4 删绑定明细）**：ShadowBuildPass 删 2 个手工 bind（Occluder SSBO + SDF image）、LightCullingPass 删 6 个手工 BindBufferBase、ShadowResolvePass 删 `ApplyComputeToFragmentBarrierTemplate`，graph 激活时走 `AreActivePassBindingsAdmitted()` fail-closed 守卫、standalone（`activeGraph==nullptr`）路径保留手工绑定 fallback；B2/B3/B4 其余手工 barrier 与 readback 保留）
...
| B2 | ShadowBuildPass typed Setup + 移除手工 bind/barrier/flush | B1 + phase-aware barrier/import binding 合同 | [x]（phase barrier 已加；真实 ShadowBuild 逐 pass GL gate 已过；2026-08-06 删除 2 个生产手工 bind：`BindBufferBase(kOccluderBinding)`+`BindImageTexture(kSdfImageBinding)`，graph 激活时 fail-closed 守卫、standalone 保留手工绑定 fallback；保留 DispatchComputeNoBarrier/phase barrier/fallback barrier） |
| B3 | ShadowResolvePass typed Setup + 移除手工 barrier/FBO 恢复 | B1 + backing/import/binding 合同 | [x]（Setup 已加；2026-08-06 删除 `ApplyComputeToFragmentBarrierTemplate()`（SDF Compute→Fragment 现由 graph transition 生成）+ include RenderSyncContracts.hpp；保留 FBO/viewport/sampler bind/quad/hdr 恢复） |
| B4 | LightCullingPass typed Setup + 移除手工 BindBufferBase/barrier/flush | B1 + B9 | [x]（Setup/graph binding 已接线；真实 LightCulling 逐 pass GL gate 已过；生产 Setup observer 接线已完成；2026-08-06 删除 6 个生产手工 `BindBufferBase`，graph 激活时 fail-closed 守卫、standalone 恢复 BindingRegistry::TryResolve 6 点手工绑定 fallback；保留 DispatchComputeNoBarrier/readback MemoryBarrier 0x2000/readback） |
...
### B2-B4 删绑定明细：生产手工绑定删除 + standalone fallback（2026-08-06）

**目标**：在 B2/B3/B4 合同级真实 GL gate 均通过、生产 Setup observer 接线完成后，删除 ShadowBuildPass/LightCullingPass 生产手工 `BindBufferBase`/`BindImageTexture` 与 ShadowResolvePass 手工 barrier，改为 graph-driven binding（`AreActivePassBindingsAdmitted()` fail-closed）。生产视觉/资源快照/GO 证据（B8）仍缺失，故**删绑定本身不再作为阻塞项**，但视觉级最终验证保留在 B8。

**改动（6 个文件）**：
- `src/engine/render/graph/RenderContext.hpp`：新增 `AreActivePassBindingsAdmitted() const`（委托 activeGraph 的 `AreActivePassBindingsAdmitted()`；activeGraph==nullptr 时返回 false 语义由调用方区分）。`RenderGraph.cpp` 同步（委托实现）。
- `src/engine/render/passes/ShadowBuildPass.cpp`：删除手工 `BindBufferBase(kOccluderBinding)`+`BindImageTexture(kSdfImageBinding)`；改为 `if (context.activeGraph != nullptr) { if (!context.AreActivePassBindingsAdmitted()) { rlDisableShader(); ReportFailure("graph-driven binding denied..."); return; } } else { 恢复 BindBufferBase(kOccluderBinding)+BindImageTexture(kSdfImageBinding) }`。保留 DispatchComputeNoBarrier/rlDisableShader/EmitPhaseBarrier+fallback MemoryBarrier(kShadowBuildBarrierBits)。
- `src/engine/render/passes/LightCullingPass.cpp`：删除 6 个手工 `BindBufferBase`（原 L372-382）+ 顶部 binding resolve/lightBufferId 检查块；改为同款守卫：graph 激活时 fail-closed（`AreActivePassBindingsAdmitted()` false → ReportFailure+return），standalone（`activeGraph==nullptr`）恢复 `BindingRegistry::TryResolve` 6 点（LIGHT_LIST_IN=0/CLUSTER_HEADER_OUT=1/CLUSTER_INDEX_OUT=2/CLUSTER_LIGHT_OUT=5/LIGHT_BOUNDS_IN=3/CLUSTER_COUNTER=4）+ lightBufferId 检查 + 6 个 `BindBufferBase`。保留 DispatchComputeNoBarrier/rlDisableShader/readback MemoryBarrier(0x2000)/readback。
- `src/engine/render/passes/ShadowResolvePass.cpp`：删除 `core::ApplyComputeToFragmentBarrierTemplate()`（SDF Compute→Fragment 现由 graph transition 生成）+ 删除 `#include "engine/render/core/RenderSyncContracts.hpp"`；保留 FBO/viewport、sampler texture bind（BindTexture GL_TEXTURE_2D）、FullscreenQuad、hdr FBO 恢复。
- `tests/integration/GraphBindingEquivalenceGLTest.cpp`：gate 语义更新（帧 A 空 snapshot → allAdmitted=false + 零 ops + "no imported backing snapshot" 诊断 + 不再断言与 manual 输出相等；帧 B 有效 snapshot → graph-driven-only 输出非 sentinel；standalone 手工绑定路径单独验证）。

**回归修复（ClusteredLightingIntegrationTest）**：删绑定后 `ClusteredLightingIntegrationTest.cpp:180 CHECK(overflowA > 0u)` 与 `:495 CHECK(cullingPass.GetLastOverflowCount() > 0u)` 失败。根因：该测试直接 `LightCullingPass cullingPass; cullingPass.Execute(context)`（不经过 RenderGraph，`activeGraph==nullptr`），删绑定后 standalone 路径无任何 SSBO 绑定 → compute 读到未绑定 → overflow=0。修复：standalone 路径恢复手工绑定 fallback（与 ShadowBuild phase-barrier fallback 先例一致），graph 路径 fail-closed 保留。修复后 2 个失败消除。

**验证证据**：
- build 成功；`ctest -L integration` 147 cases 145 passed 2 failed（ClusteredLighting 回归消除，仅剩 known unrelated `GPUEntityLifecycleRegistryTest` W5）；`ctest -L unit` 9/9；`ctest -R "nmd.tests.gpu.contract|nmd.tests.gpu.diagnostic"` 2/2；`check_module_boundaries.py` PASS（0/0）；`check_legacy_reintroduction.py` PASS（219/70）；`git diff --check` exit 0（仅 CRLF 提示）。
- grep：6 个 C pass 内 `GPUUtils::MemoryBarrier` 生产调用点 0；src 内 `ShaderHotReloadManager` 引用 0。

**残余（不阻塞）**：生产视觉/资源快照/GO 证据仍在 B8；B2/B3/B4 其余手工 barrier（ShadowBuild fallback barrier、LightCulling readback MemoryBarrier 0x2000、ShadowResolve 原 barrier 外的 host readback 语义）保留。
...
- B2-B4: Setup、import、snapshot 和 graph-driven BufferBase/ImageUnit 准入均可验证；真实 GL gate 通过后，才允许逐项移除对应手工 `BindBufferBase`/`BindImageTexture`，并保留 ShadowBuild phase barrier 与 ShadowResolve 必需屏障直到等价性证据成立。**B2/B3/B4 已按此完成（2026-08-06，见 B2-B4 删绑定明细）**：合同级真实 GL gate 通过后删除生产手工 bind；ShadowBuild phase barrier、LightCulling readback MemoryBarrier、ShadowResolve 必需屏障保留。
...
- **B2-B4 删绑定验证证据（2026-08-06）:** 见 B2-B4 删绑定明细。`cmake --build build --config RelWithDebInfo --target NoMoreDayTests -- /m:2` 成功；`ctest -L integration` 147 cases 145 passed 2 failed（ClusteredLighting 回归消除，仅剩 known unrelated `GPUEntityLifecycleRegistryTest` W5）；`ctest -L unit` 9/9；`ctest -R "nmd.tests.gpu.contract|nmd.tests.gpu.diagnostic"` 2/2；`check_module_boundaries.py` PASS（0/0）；`check_legacy_reintroduction.py` PASS（219/70）；`git diff --check` exit 0。B2/B3/B4 状态更新为 [x]。
