...
## Batch 4-D — HeightFieldAdapter (GlobalHeightField + HeightShadowPass Game edges cleared)

Date: 2026-08-02. Scope: split the ECS -> height-field stamp projection out of `src/engine/render/lighting/GlobalHeightField.cpp` into a Game-side adapter, narrow the Engine `GlobalHeightField` to a span-driven texel rasterizer, Config-inject the world semantics into `HeightShadowPass`, and delete all 4 Engine->Game ledger edges (`GlobalHeightField.cpp:4:Common.hpp`, `:5:MapComponent.hpp`, `:6:ShadowCasterComponent.hpp` + `HeightShadowPass.cpp:11:Common.hpp`). 4 ledger edges removed (15 -> 11). Uncommitted working tree at HEAD `bf5ff63`.

### Changes

- **NEW `src/game/render/HeightFieldAdapter.hpp` + `.cpp`** (namespace `NoMoreDay`, modeled on the LightAdapter/OccluderProjector split-adapter precedents): `HeightFieldAdapter::BuildStamps(entt::registry&) -> HeightFieldProjection {std::vector<render::lighting::GlobalHeightField::HeightStamp> stamps; float worldWidth/worldHeight/tileWorldSize;}`. Projects the 5 ECS sources from the old `BuildTerrainAndStatic`/`BuildDynamicLayer` loops into pure stamps: (1) `MapTileComponent` -> `Kind::Tile` stamps (WALL->0.85 / else->0.10, static); (2) static `ShadowCasterComponent` (dynamicFlag==0) -> Disc @(pos.x,pos.y) r=20 height=clamp(occluderHeight), static; (3) `ColliderComponent::Static` -> Disc r=max(2,max(w,h)*0.5) height=0.75, static; (4) dynamic casters -> Disc r=18, dynamic; (5) `SpriteComponent` -> Disc r=max(6,8*max(0.25,scale)) height=EstimateMaskBlue (skipped when blue<=0.02), dynamic. `EstimateMaskBlue` + per-texture cache moved **verbatim** from `GlobalHeightField.cpp:26-52` (now a `static thread_local std::unordered_map` in the adapter). `worldWidth/Height` = `Constants::World::WORLD_WIDTH/HEIGHT` (int->float), `tileWorldSize` = `GRID_TILE_SIZE`.
- `src/engine/render/lighting/GlobalHeightField.hpp`: added the nested `HeightStamp` DTO (`Kind{Tile,Disc}; tileX/tileY or worldX/worldY/worldRadius; height; dynamic`); `Update(entt::registry&)` -> `Update(std::span<const HeightStamp>)`; `BuildTerrainAndStatic`/`BuildDynamicLayer` re-signatured to `std::span<const HeightStamp>` (split by `stamp.dynamic`; static stamps rasterize into `m_baseLayer`, dynamic into `m_dynamicLayer`); dropped `#include <entt/entt.hpp>` + `<unordered_map>` (removed `m_maskBlueCache`, moved to adapter), added `<span>`; `Config` drops `terrainWallHeight`/`terrainFloorHeight` (now adapter constants 0.85/0.10) and gains `float tileWorldSize = 10.0f` (clamped >=1 in `Initialize`, replaces the game `Constants::World::GRID_TILE_SIZE` in `StampTileRectMax`).
- `src/engine/render/lighting/GlobalHeightField.cpp`: deleted the 3 game includes (`Common.hpp`/`MapComponent.hpp`/`ShadowCasterComponent.hpp`) + `EstimateMaskBlue`; `Update`/`BuildTerrainAndStatic`/`BuildDynamicLayer` consume the stamp span; full-rebuild semantics preserved (static pass clears both layers, fills all dirty chunks, clears prevDynamicChunks; dynamic pass re-stamps the dynamic layer and tracks dynamic chunks for incremental clear). Chunk compose/upload/WorldToTexel/StampDiscMax/StampTileRectMax/SampleNormalizedHeight unchanged.
- `src/engine/render/passes/HeightShadowPass.cpp`: deleted `#include "game/components/Common.hpp"` (its only use was the `Constants::World` world-size Config at old :176-177); dropped the `context.registry == nullptr` guard (per Batch 4-B precedent — the pass no longer reads the registry); Config world/tile size now injected from `graph::RenderContext` (`cfg.worldWidth/worldHeight/tileWorldSize` from `context.worldWidth/worldHeight/tileWorldSize`, fallback 5000/5000/10 when the game layer provides 0); `m_heightField.Update` now feeds `std::span<const lighting::GlobalHeightField::HeightStamp>(context.heightFieldStamps, context.heightFieldStampCount)` (spans over nullptr are empty). The pass keeps the DTO entry point + texture work.
- `src/engine/render/graph/RenderContext.hpp`: `#include "engine/render/lighting/GlobalHeightField.hpp"` (HeightStamp is nested -> cannot forward-declare; engine->engine include, OK); added `const lighting::GlobalHeightField::HeightStamp* heightFieldStamps = nullptr; uint32_t heightFieldStampCount = 0u;` + `float worldWidth/worldHeight/tileWorldSize = 0.0f;` (pointer+count+scalar DTO style, matching the occluder fields). `registry` field **kept** (still used by `RadianceCascadesPass`).
- `src/engine/render/GameplayRenderHooks.hpp`: `GameplayRenderFrame` gains Engine-owned `std::vector<lighting::GlobalHeightField::HeightStamp>* heightFieldBuffer` (after `lightBuffer`) + out-fields `worldWidth/worldHeight/tileWorldSize`; new pure-virtual `onHeightField(GameplayRenderFrame&)` (only implementer is `GameplayRenderAdapter`).
- `src/game/render/GameplayRenderAdapter.hpp/.cpp`: implements `onHeightField` — calls `HeightFieldAdapter::BuildStamps(frame.registry)`, moves stamps into `*frame.heightFieldBuffer`, copies the 3 world out-fields.
- `src/engine/render/RenderSystem.cpp`: static `s_heightFieldBuffer` (engine-owned, mirrors `s_occluderBuffer`/`s_lightCandidateBuffer`); `RenderFrameData` gains `worldWidth/worldHeight/tileWorldSize`; `ToHooksFrame()` passes `&s_heightFieldBuffer` + the 3 world fields; the hooks block calls `gameplayHooks->onHeightField(hooksFrame)` after `onOccluders` (or clears `s_heightFieldBuffer` + zeros the world fields for gate/harness null-hooks); graphContext assembly fills `heightFieldStamps`/`heightFieldStampCount` from `s_heightFieldBuffer` + the world fields. **Graph build structure (AddPass order/conditions/owner tags/composite input selection) zero modification.**
- Ledger: deleted exactly 4 entries -> **11 entries** = LegacyLowerPch -> Game (MS-7) 5 + NoMoreDayEngine -> Game (MS-6) 6. Checker: removed `src/engine/render/lighting/GlobalHeightField.cpp` + `src/engine/render/passes/HeightShadowPass.cpp` from `REQUIRED_P0_SOURCES` (0 residual edges each, per precedent).
- `tests/unit/GlobalHeightFieldTest.cpp`: rewritten as stamp-driven (no registry). Test 1 "terrain/static compose" builds `Kind::Tile` stamps floor(1,1)=0.10 + wall(5,5)=0.85, same Config 128x128/chunk16/world1280, asserts `wallH > floorH` + `didFullRebuild`. Test 2 "dynamic chunk incremental update" builds a dynamic Disc @(200,200) r=18 h=0.95, same Config 256x256/chunk32/world512, then a second Update with only a fresh Disc @(340,330) (mirroring the adapter's per-frame rebuild), asserts `newSpot > oldSpot` + `dirtyChunkCount > 0`. **Correctness anchor: `SampleNormalizedHeight` values unchanged** (identical texel math + identical stamp parameters).
- `tests/python/ModuleBoundaryCheckerTest.py`: unchanged (uses `GPULootSystem.cpp` as the P0 source, still in `REQUIRED_P0_SOURCES`).
- No CMake edits needed (`GLOB_RECURSE CONFIGURE_DEPENDS` auto-picks the new `.cpp`).

### Verification (real output)

1. `python scripts/check_module_boundaries.py` -> `[Module Boundary] Observed/ledger edges: 11/11; files: 5` and `PASS` (exit 0). Breakdown: LegacyLowerPch -> Game (MS-7) 5, NoMoreDayEngine -> Game (MS-6) 6.
2. `python -m unittest tests/python/ModuleBoundaryCheckerTest.py -v` -> 6 tests OK (exit 0; test_repository_baseline_passes runs the real-repo checker).
3. Full build redirected to `C:\Users\yuminao\AppData\Local\Temp\opencode\ms6-b4d-build.log` -> EXIT=0, both success markers (`[Build] Build completed successfully.` / `[Build] All steps completed successfully`), 0 error C/LNK/FAILED. First attempt failed with `error C2039: "size_hint": not a member of entt basic_view` at `HeightFieldAdapter.cpp:68` (const single-type view has no `size_hint`) -> fixed by `view.size()`; second build clean.
4. Targeted `bin\NoMoreDayTests.exe --test-case="*GlobalHeightField*,*HeightShadow*,*Clustered Lighting*"` -> `test cases: 11 | 11 passed | 0 failed | 700 skipped`, `assertions: 28680 | 28680 passed | 0 failed`, `Status: SUCCESS!`, exit 0. Isolated `--test-case="*GlobalHeightField*"` -> `test cases: 2 | 2 passed | 0 failed | 709 skipped`, `assertions: 7 | 7 passed | 0 failed`, `Status: SUCCESS!` (log: `C:\Users\yuminao\AppData\Local\Temp\opencode\ms6-b4d-tests.log`).
5. `git grep -n "game/" -- src/engine/render/lighting/GlobalHeightField.cpp src/engine/render/passes/HeightShadowPass.cpp` -> 0 hits (exit 1).
6. `git diff --check` -> exit 0 (CRLF warnings only).

### Deferred

- Remaining MS-6 edges (Batch 7): GPULoot 2 + GPUSkillEffect 1 + RadianceCascadesPass 1 + VFX 2 = 6; plus 5 pch (MS-7). GlobalHeightField + HeightShadowPass are fully Game-edge-free.
- RG-3 resource lifecycle untouched; graph construction (AddPass order/conditions/owner tags) zero-modification verified.
- Nothing was staged or committed (per project rule; main agent commits).

---

## Batch E — EmissiveStampAdapter (last MS-6 edge cleared, REQUIRED_P0_SOURCES emptied)

Date: 2026-08-02. Scope: split the ECS material-emissive stamp projection out of `RunMaterialEmissive` (`src/engine/render/passes/RadianceCascadesPass.cpp`) into a Game-side `EmissiveStampAdapter`, have the pass consume a `graph::RenderContext` span instead of the registry, and delete the final MS-6 ledger edge (`RadianceCascadesPass.cpp:18:game/components/Common.hpp`). 1 ledger edge removed (6 -> 5). Uncommitted working tree at HEAD `9aece25`.

### Changes

- **NEW `src/engine/render/GPUData.hpp` DTO `components::EmissiveStampInput`** (CPU-side span DTO, no static_assert needed): `Vector2 worldPos; float worldHalfExtent; int maskLayer; Vector4 emissionRGBA;` (RGBA.w == emissive intensity).
- **NEW `src/game/render/EmissiveStampAdapter.hpp` + `.cpp`** (namespace `NoMoreDay`, modeled on the LightAdapter/HeightFieldAdapter/GPULootAdapter precedents): `EmissiveStampAdapter::BuildEmissiveStamps(entt::registry&) -> EmissiveProjection {std::vector<components::EmissiveStampInput> stamps;}`. Moved **verbatim** from old `RunMaterialEmissive`: `registry.view<const Position, const vfx::ActiveMaterialSwap>(entt::exclude<KilledTag>)` loop, `swap.materialId <= 0` skip, `MaterialManager::Get().GetGpuMaterialForTesting(swap.materialId)`, `maskLayer = lround(textureSlots.z)` (< 0 skip), emissive R/G/B + intensity (clamp >= 0; skip when intensity <= 0.0001f or R+G+B <= 0.0001f), `worldHalfExtent = max(24, Radius->value, sprite half-extent)` (SpriteComponent + texture.id != 0 branch). `reserve(view.size_hint())` (single-type multi-component const view supports it, per LightAdapter/OccluderProjector). Includes only: `game/render/EmissiveStampAdapter.hpp`, `engine/render/MaterialManager.hpp`, `engine/vfx/VFXTypes.hpp`, `game/components/Common.hpp`. `MaterialManager` fully qualified as `NoMoreDay::render::MaterialManager` (unqualified would not resolve from `namespace NoMoreDay`).
- `src/engine/render/passes/RadianceCascadesPass.cpp`: deleted 3 includes (`engine/render/MaterialManager.hpp`, `engine/vfx/VFXTypes.hpp`, `game/components/Common.hpp`); kept `<entt/entt.hpp>` (only for `using namespace entt::literals` `_hs` in Initialize). `RunMaterialEmissive` guard drops `context.registry == nullptr` (pass no longer reads the registry); body now iterates `context.emissiveStamps[0..emissiveStampCount)` and keeps world->px conversion + per-stamp dispatch: `halfExtentPixels = max(2, ceil(stamp.worldHalfExtent * zoom))`, `dispatchSize/centerPx/dispatchOrigin`, offscreen skip, `uMaskLayer/uDispatchOrigin/uDispatchSize/uEmission` uniforms (emission = `{stamp.emissionRGBA.x..w}`), `DispatchComputeNoBarrier` + `MemoryBarrier(Image)`, `m_lastMaterialStampCount = stampCount`. All GPU/barrier/RG-3 resource code untouched.
- `src/engine/render/graph/RenderContext.hpp`: added `struct EmissiveStampInput;` forward-decl next to `GPUShadowCaster` and fields `const components::EmissiveStampInput* emissiveStamps = nullptr; uint32_t emissiveStampCount = 0u;` (pointer+count, after heightFieldStampCount). `registry` field kept.
- `src/engine/render/GameplayRenderHooks.hpp`: `GameplayRenderFrame` gains Engine-owned `std::vector<NoMoreDay::components::EmissiveStampInput>* emissiveStampBuffer = nullptr;` (after `lootBuffer`); new pure-virtual `onEmissive(GameplayRenderFrame&)` after `onLoot` (only implementer `GameplayRenderAdapter`).
- `src/game/render/GameplayRenderAdapter.hpp/.cpp`: implements `onEmissive` — `if (frame.emissiveStampBuffer == nullptr) return;` then `EmissiveStampAdapter::BuildEmissiveStamps(frame.registry)` moved into `*frame.emissiveStampBuffer`.
- `src/engine/render/RenderSystem.cpp`: static `s_emissiveStampBuffer` (engine-owned); `ToHooksFrame()` inserts `&s_emissiveStampBuffer` after `&s_lootInstanceBuffer` (aggregate order = member order); hooks block calls `gameplayHooks->onEmissive(hooksFrame)` after `onLoot`; gate/harness else-branch adds `s_emissiveStampBuffer.clear()`; graphContext assembly adds `graphContext.emissiveStamps`/`emissiveStampCount` from `s_emissiveStampBuffer` (after heightField fields). **Graph build structure (AddPass order/conditions/owner tags) zero modification.**
- Ledger: deleted exactly 1 entry (the RadianceCascadesPass Common edge) -> **5 entries** = 5 pch (MS-7). Note: the ledger recorded `:18` which is the correct current include line (the task brief's `:19` premise was a misread of the source); since the edge was deleted outright, no line correction was needed.
- `scripts/check_module_boundaries.py`: `REQUIRED_P0_SOURCES = frozenset({"src/engine/render/passes/RadianceCascadesPass.cpp"})` -> `frozenset()`. Confirmed the empty frozenset is tolerated: `source in REQUIRED_P0_SOURCES` is always False (required-source assertion never fires) and `source not in REQUIRED_P0_SOURCES` is always True (so any entry still carrying `p0_blocking == P0_BLOCKER` is rejected as outside policy — correct post-MS-6). This is the last MS-6 P0 source; the set is now empty by design.
- `tests/python/ModuleBoundaryCheckerTest.py`: `test_required_p0_source_without_blocker_returns_input_error` REPLACED by `test_empty_required_p0_sources_tolerated_after_ms6` — same temp-repo fixture (RadianceCascadesPass.cpp with `#include "game/components/Common.hpp"` + tracked ledger entry, p0_blocking None) now expects returncode 0 + `PASS` (the old "source requires" branch can no longer fire with an empty REQUIRED set). Other 5 tests unchanged.
- `tests/integration/GIStabilityIntegrationTest.cpp` (listed as a batch test to refactor): added `#include "game/render/EmissiveStampAdapter.hpp"`, a `std::vector<EmissiveStampInput> emissiveStamps;` next to the pass, and per-frame before `pass.Execute` the adapter projection (`BuildEmissiveStamps(registry)` -> move -> `context.emissiveStamps`/`emissiveStampCount`). **Also added `pass.PrepareVfxEmissionSnapshot(context)`** before Execute: this test was already failing at HEAD (verified by stash+rebase baseline run: 1718 assertions / 38 passed / 1680 failed) because commit `5c5ca0c` (`dto render input and rewire pass resources`) stubbed `RunParticleEmissive` to return `m_particleEmissive.IsValid() && m_vfxEmissionSnapshotValid`, and Execute aborts at that step when the snapshot was never prepared (the benchmark test calls it, the GI test did not). With the call added the test passes 1718/1718. This is a test-only synchronization fix, no production-code change.
- `tests/performance/RadianceCascadesBenchmark.cpp`: added `#include "game/render/EmissiveStampAdapter.hpp"`, `std::vector<EmissiveStampInput> emissiveStamps;` after the pass decl, and after each `PopulateScenario` the adapter projection feeding `context.emissiveStamps`/`emissiveStampCount` (lives for the 4 MeasureRadianceTier calls).
- `tests/integration/MaterialLightingIntegrationTest.cpp:56` — **no code change needed**: verified via git log that this file never referenced `RadianceCascadesPass` or `pass.Execute`; its line 56 (`auto &particleSystem = systems::GPUParticleSystem::Get();`) is a GPUParticleSystem tier-ABI + hot-reload test unrelated to the material-emissive projection. Confirmed by build + targeted runs.
- No CMake edits needed (`GLOB_RECURSE CONFIGURE_DEPENDS` auto-picks the new `.cpp`).

### Verification (real output)

1. `python scripts/check_module_boundaries.py` -> `[Module Boundary] Observed/ledger edges: 5/5; files: 1` and `PASS` (exit 0). Breakdown: LegacyLowerPch -> Game (MS-7) 5. MS-6 edge count is now 0.
2. `python -m unittest tests/python/ModuleBoundaryCheckerTest.py -v` -> 6 tests OK (exit 0; includes the new empty-REQUIRED tolerance test).
3. Full build redirected to `C:\Users\yuminao\AppData\Local\Temp\opencode\ms6-b4e-build.log` -> EXIT=0, both success markers (`[Build] Build completed successfully.` / `[Build] All steps completed successfully`), 0 error C/LNK/FAILED. First attempt failed with `error C2653: "MaterialManager": not a class or namespace` at `EmissiveStampAdapter.cpp:29` (unqualified `MaterialManager` from `namespace NoMoreDay`; the class lives in `NoMoreDay::render`) -> fixed by `NoMoreDay::render::MaterialManager::Get()`; second build clean.
4. Targeted tests (real output):
   - `--test-case="[Integration] GI - Long-run Stability Proxy (Resize + Tier Switch)"` -> `test cases: 1 | 1 passed | 0 failed`, `assertions: 1718 | 1718 passed`, `Status: SUCCESS!`
   - `--test-case="[Performance] RadianceCascades - Tier and Holographic Matrix"` -> `test cases: 1 | 1 passed`, `assertions: 3869 | 3869 passed`, `Status: SUCCESS!` (RELEASE_GATE_METRIC lines present)
   - `--test-case="[Integration] Material Lighting*"` -> `2 | 2 passed`, `21 | 21 passed`, SUCCESS
   - `--test-case="[Performance] Material Lighting - Tier overhead budgets"` -> `1 | 1 passed`, `5 | 5 passed`, SUCCESS
   - `ctest --test-dir build -C RelWithDebInfo -L integration` -> 6/6 passed (incl. module-gate 5 tests)
   - `ctest --test-dir build -C RelWithDebInfo -L ci` -> 641 cases / 639 passed / 2 failed: the 2 failures are **pre-existing and unrelated** — `HeavenlySwordClosureTests.cpp:97 CHECK(hasFreeze)` (chance-based flaky, documented in evidence since Batch 2) and `UITests.cpp:438` (checks UISkillHub.cpp source text for a `id == 10 || id == 11 || id == 12` literal; UISkillHub.cpp is byte-identical to HEAD and also lacks the string at HEAD — pre-existing, zero files from this batch involved).
5. `git grep -n "game/" -- src/engine/render/passes/RadianceCascadesPass.cpp` -> 0 hits (exit 1).
6. `git diff --check` -> exit 0 (CRLF warnings only).

### Deferred

- Remaining ledger edges: 5 pch (MS-7) only. **All MS-6 edges cleared; `REQUIRED_P0_SOURCES` is now an empty frozenset** (confirmed the checker tolerates it; any future entry with `p0_blocking == P0_BLOCKER` is rejected as outside policy, which is correct post-MS-6).
- RG-3 resource lifecycle untouched (RadianceCascadesPass buffer/shader/texture create/destroy registration byte-identical to HEAD); graph construction (AddPass order/conditions/owner tags) zero-modification verified; gate/empty-hooks path verified (else-branch clears `s_emissiveStampBuffer`, graphContext span stays null/0 for harness runs).
- Nothing was staged or committed (per project rule; main agent commits).
