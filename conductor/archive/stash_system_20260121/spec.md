...
public:
    // --- 物品转移 ---
    // source/dest 可以是: Inventory, PersonalStash[tab], SharedStash[tab]
    // 返回 true 如果转移成功
...
```json
{
  ...
  "personal_stash": {
    ...
    "tabs": [
      ...
      { "name": "Runes", "type": 3, "icon_id": 12, "color": 4294967295, "items": [ ... ] },
      { "name": "Junk", "type": 0, "icon_id": 0, "color": 4294967295, "items": [ ] }
    ]
  }
}
```
...
```json
{
  ...
  "shared_stash": {
    ...
    "tabs": [
      ...
      { "name": "Trade Stock", "type": 0, "icon_id": 8, "color": 4278255360, "items": [ ... ] }
    ]
  }
}
```
...
复用现有 `SerializedItem` DTO，无需修改物品序列化逻辑。
...
struct SerializedStashTab {
    ...
    uint32_t color;
    // 稀疏存储: 只保存非空槽位
    std::vector<std::pair<int, SerializedItem>> items; // (slotIndex, itemDTO)
};
...
struct StashInteractableComponent {
    StashType type;  // Personal 或 Shared
};
...
| 元素 | 样式 |
|------|------|
| **锁定标签页** | 灰色 + 🔒图标，点击时显示解锁费用确认弹窗 |
| **当前选中页** | 高亮边框 + 微缩放动画 |
| **物品悬停** | 复用 `UITooltip` 组件显示物品详情 |
| **拖拽物品** | 半透明跟随鼠标，目标槽位高亮 |
| **无效放置** | 目标槽位红色闪烁 (尝试存入 Material 时显示提示) |
| **搜索匹配** | 匹配物品高亮边框，非匹配物品半透明 |
| **城镇仓库实体** | 初期为棕色矩形占位符 (64x64)，后续替换为精灵资源 |

### 5.5 搜索功能规格

| 属性 | 说明 |
|------|------|
| **搜索范围** | 当前仓库类型 (Personal/Shared) 的所有 **已解锁且非空** 的标签页 |
| **匹配字段** | 物品名称、词缀名称、底材名称 |
| **结果展示** | 高亮所有匹配物品，当前标签页优先显示 |
| **跳转支持** | 点击搜索结果可跳转到对应标签页和槽位 |
...
| 风险 | 影响 | 缓解措施 |
|------|------|----------|
| **物品复制** | 玩家利用网络延迟或崩溃复制物品 | 仓库操作采用"移除后再添加"的原子流程；存盘前校验物品 instanceId 唯一性 |
| **存档膨胀** | 每个标签页最多存储 144 个完整物品 DTO | 采用稀疏存储 (仅序列化非空槽位)；JSON 体积控制在 < 500KB/仓库 |
| **性能问题** | 共享仓库被多角色频繁读写 | 共享仓库采用"脏标记+延迟写入"策略；城镇退出时一次性保存 |
| **数据迁移** | 旧存档缺少 stash 字段 | `SaveManager::loadCharacter` 检测缺失字段时创建默认空仓库 |
| **错误存入** | 玩家尝试存入 Material 物品 | `canStoreItem()` 检查物品类型，Material 返回 false 并显示 UI 提示 |
...
- [ ] 玩家可在城镇地图中与仓库实体交互打开仓库界面
- [ ] 玩家可以通过金币解锁新的标签页 (费用按表收取)
- [ ] 玩家可以拖拽物品在 背包 <-> 仓库 之间移动
- [ ] **Material 类型物品无法存入仓库**，尝试时显示提示
- [ ] Ctrl+点击 实现快速存取 (Material 物品除外)
- [ ] 标签页支持重命名 (最长 16 字符)
- [ ] **搜索功能** 可搜索当前仓库类型的所有已解锁非空标签页
- [ ] 共享仓库数据在多角色之间持久化
- [ ] 存盘/读档后仓库数据完整无丢失

### 7.2 性能验收

- [ ] 打开仓库界面 < 50ms (不含物品纹理加载)
- [ ] 标签页切换 < 16ms (无感知卡顿)
- [ ] 满仓库存盘 (10页 x 144物品 x 2仓库) < 500ms (异步, 不阻塞主线程)
...
| 问题 | 决策 |
|------|------|
| **物品类型限制** | Material 类型物品 **不能** 存入仓库 (存储在专用材料袋) |
| **搜索范围** | 仅搜索 **当前仓库类型** 的所有已解锁非空标签页，不跨仓库搜索 |
| **标签页排序** | **不支持** 拖拽排序标签页 |
| **访问方式** | 通过城镇地图中的仓库实体交互，而非快捷键 |
| **初期渲染** | 城镇仓库实体渲染为简单矩形占位符，后续迭代添加资源 |
