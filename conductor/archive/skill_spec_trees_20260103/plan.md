...
## Phase 2: Flowing Thrust (流云刺) Specialization Implementation [checkpoint: eca5fcc]
...
## Phase 3: Rending Wave (裂空斩) Specialization Implementation [checkpoint: eca5fcc]
...
- [x] Task: 运行全局集成测试，检查两个技能在不同专精组合下是否会导致崩溃或逻辑冲突。
- [x] Task: 执行性能基准测试，验证大量实体（10,000+）在触发复杂专精逻辑（如多重投射物和残影）时的 FPS 稳定性。
- [x] Task: 清理调试日志，完善代码注释，并根据 `code_styleguides/` 进行代码规范检查。
