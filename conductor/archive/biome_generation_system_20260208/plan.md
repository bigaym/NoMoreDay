...
|------|---------|------|----------|----------|------|
| **Phase 1** | 1.x | 数据驱动层 | BiomeConfig扩展, JSON解析, 枚举定义 | 1 天 | ✅ 已完成 |
| **Phase 2** | 2.x | 渲染与物理增强 | 空气墙渲染器, 背景Shader, 特殊物理 | 2 天 | ✅ 已完成 |
| **Phase 3** | 3.x | 生成算法演进 | 三类策略生成器, CA参数调优 | 2-3 天 | ✅ 已完成 |
| **Phase 4** | 4.x | 动态交互逻辑 | 可破坏地形, 动态刷怪墙, 加速带 | 1.5 天 | ✅ 已完成 |
| **Phase 5** | 5.x | 生态集成 | 怪物池映射, 掉落表关联 | 1 天 | ✅ 已完成 |
| **Phase 6** | 6.x | 测试与打磨 | 全群落测试, 性能基准, Bug修复 | 1 天 | ✅ 已完成 |
...
- [x] 测试 B组地图走廊连通性
- [x] 测试 C组地图空气墙正确标记
- [x] 测试所有地图 `EnsureConnectivity()` 通过
