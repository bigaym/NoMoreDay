...
- [x] **任务 3.2**：编写 `DamagePipelineTest.hpp`，预设复杂的转换场景（例如：50% 物理转火焰，应用两层加成）。 [653d2f3]
- [x] **任务 3.3**：实现第一步（基础池）和第二步（转换/附加）逻辑。 [653d2f3]
- [x] **任务 3.4**：实现第三步（Increased）和第四步（More）逻辑，确保修饰符按优先级应用。 [653d2f3]
- [x] **任务 3.5**：实现第五步（最终结算），集成暴击和防御占位。 [653d2f3]
