...
| 阶段 | 名称 | 核心产出 | 状态 |
|------|------|----------|------|
| **Phase 1** | Emissive Buffer | 发光体投影 + Buffer 管理 | [x] |
| **Phase 2** | 级联原型 | 4 级联 + half-res + 基础合并 | [x] |
| **Phase 3** | GI 合成 | RadianceMap → LitHDR 叠加 + 时域混合 | [x] |
| **Phase 4** | 完整体 | 6 级联 + full-res + 优化 | [x] |
| **Phase 5** | 探索 & 验证 | Holographic RC 评估 + 全链路验证 | [x] |
...
- [x] Task 1.3: 材质 Emission 通道 → Emissive Buffer（从 GPUMaterialDataV3 Mask.A 读取）
...
### Verification
- [x] 发光体在 Emissive Buffer 中可辨（debug 视图）
- [x] VFX 与灯光写入无竞争
...
### Verification
- [x] 4 级联 GI 在洞穴场景呈现间接光照
- [x] L0 输出 RadianceMap 非全黑/全白
...
- [x] Task 4.4: Ultra 档 vs High 档 GI 质量对比
- [x] Task 4.5: 帧预算腾挪验证（V4 Pass 联合降级）
- [x] Task 4.6: 6 级联 ≤ 2.5ms @1080p 性能验证

### Verification
- [x] 6 级联颜色溢出（color bleeding）可辨
- [x] Ultra 常规场景 ≥ 180 FPS
...
- [x] Task 5.2: Holographic RC vs 标准 RC 性能/质量对比
- [x] Task 5.3: OGL 4.3 兼容性评估
- [x] Task 5.4: 全 Tier 矩阵验证（Low/Med=Off, High=4-cascade, Ultra=6-cascade）
- [x] Task 5.5: 3+ 场景类型 GI 视觉回归
- [x] Task 5.6: GI 长时间运行稳定性（30 分钟无显存增长）

### Verification
- [x] Holographic RC 可行性结论
- [x] 全部 AC 达成
