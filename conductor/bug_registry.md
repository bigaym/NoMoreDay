...
| BUG-20260214-001 | 2026-02-14 | 手测 | P2 | Open | Debug 构建下点击“开始游戏”后进程异常退出（产生 `.dmp`），RelWithDebInfo 暂未复现 | 1) 使用 Debug 构建启动；2) 进入主菜单点击开始游戏；3) 观察到异常退出并生成 `crashes/crash_2026-02-14_01-11-03.dmp` | 当前仅在 Debug 构建观察到；RelWithDebInfo 同路径未复现 | 开发期联调稳定性；可能掩盖潜在 UB/竞态 | 未定位（日志在 Gameplay 初始化阶段中断，未见显式异常日志） | `crashes/crash_2026-02-14_01-11-03.dmp`; `logs/NoMoreDay.log`; `src/game/states/GameplayState.cpp`; `src/engine/render/RenderSystem.cpp` | 待定：先做符号化与最小复现收敛，再决定是否修复或仅做 Debug 保护 | 已确认：RelWithDebInfo 不复现；Debug 可触发并产出 dump | 建议增加一次性启动期守卫日志（状态切换边界）并保留 dump 分析脚本入口 | 建议先限时 1~2 小时做定级，不立即展开大修 |
