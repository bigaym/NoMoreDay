...
## Phase 1: 基础设施与静态数据 (Infrastructure & Data)
...
- [ ] Task: 扩展 `SkillData` 结构，增加 `charge_count`（最大充能次数）字段
- [ ] Task: 编写单元测试：验证 `BuffRegistry` 的映射检索逻辑
- [ ] Task: Conductor - User Manual Verification 'Phase 1: Infrastructure & Data' (Protocol in workflow.md)

## Phase 2: 底部技能快捷栏实现 (Skill Hotbar Implementation)
- [ ] Task: 在 `UIRenderer` 中建立 5 个底部固定槽位的绘制逻辑
- [ ] Task: 实现技能图标的“径向遮罩” (Radial Wipe) 冷却动画
- [ ] Task: 在技能图标左下角渲染蓝色的法力消耗数字小字
- [ ] Task: 在图标右下角渲染当前可用充能次数数字
- [ ] Task: 实现法力不足时的“图标暗淡”视觉状态
- [ ] Task: 编写测试：验证技能栏在不同 CD 和蓝量状态下的渲染状态切换
...
- [ ] Task: 实现资源条上方的 Buff 容器布局，支持多图标横向排列
- [ ] Task: 实现基于“文字+箭头”的图标绘制逻辑，并根据类型添加绿色/红色边框
- [ ] Task: 实现 Buff 图标上的圆形持续时间进度显示
- [ ] Task: 实现层数 (Stacks) 数字的角落渲染
...
- [ ] Task: 实现 HUD 上的鼠标碰撞检测，识别技能或 Buff 图标悬停
- [ ] Task: 实现基础 Tooltip 浮窗绘制，包含名称、详细描述和标签
