...
2. **P2-B（进行中）**：怪物行为词缀事件门禁已并轨 UMR（Update/Hit/Death），并完成 `Molten` / `Vampiric` / `Teleporter` / `Frozen` / `Nullifier` / `Entangler` / `Toxic` / `MirrorImage` / `StormStrider` / `SoulEater` 的 Op 优先分发（保留旧分支回退）。
